 ...
 *
 * Subcommands:
 *   lmas config show [--level <n>] [--app <name>] [--debug]
 *   lmas config diff --levels <a>,<b>
 *   lmas config migrate [--dry-run] [--force]
 *   lmas config validate [--level <n>]
...
// Resolve core config modules (relative from .lmas-core/cli/commands/config/)
...
function migrateAction(options) {
  ...
  try {
    ...
    if (originalStr === resolvedStr) {
      ...
      console.log('This may be expected due to key normalization. Run `lmas config diff` to inspect.');
    }
...
function initLocalAction() {
  const root = getProjectRoot();
  const templatePath = path.join(root, '.lmas-core', 'local-config.yaml.template');
  const targetPath = path.join(root, '.lmas-core', 'local-config.yaml');

  if (!fs.existsSync(templatePath)) {
    console.error('Template not found: .lmas-core/local-config.yaml.template');
    process.exit(1);
  }
  ...
  console.log('Created: .lmas-core/local-config.yaml (from template)');
...
/**
 * Create the `lmas config` command with all subcommands.
 * @returns {Command}
 */
