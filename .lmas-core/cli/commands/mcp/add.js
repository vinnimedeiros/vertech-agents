...
async function executeAdd(server, options) {
  ...
  if (options.listTemplates) {
    ...
    console.log('Usage: lmas mcp add <server-name>');
    console.log('Example: lmas mcp add context7');
    return;
  }
  ...
  if (!globalConfigExists()) {
    ...
    console.log('Run "lmas mcp setup" first to create global configuration.');
    process.exit(1);
  }
  ...
  if (!server) {
    ...
    console.log('  lmas mcp add myserver --type sse --url https://example.com/mcp');
    console.log('  lmas mcp add myserver --command npx --args "-y,@scope/mcp-server"');
    process.exit(1);
  }
  ...
  if (result.success) {
    ...
    if (result.error.includes('No template available')) {
      ...
      console.log(`  lmas mcp add ${server} --type sse --url <url>`);
      console.log(`  lmas mcp add ${server} --command npx --args "-y,@scope/server"`);
    }
