...
async function executeStatus(options) {
  ...
  if (globalExists) {
    ...
    console.log('  Run "lmas mcp setup" to create global config');
  }
  ...
  switch (linkStatus.status) {
    ...
    case LINK_STATUS.NOT_LINKED:
      ...
      console.log('  Run "lmas mcp link" to link project to global config');
      break;
    case LINK_STATUS.BROKEN:
      console.log('  Status: 🔴 Broken link');
      console.log(`  Target: ${linkStatus.target}`);
      console.log('  Run "lmas mcp link --force" to fix');
      break;
    case LINK_STATUS.DIRECTORY:
      ...
      if (projectConfig.found) {
        ...
        console.log('  Run "lmas mcp link --migrate" to use global config');
      }
      break;
    default:
  ...
  // Quick actions
  if (!globalExists) {
    console.log('\n💡 Quick start: lmas mcp setup --with-defaults');
  } else if (linkStatus.status === LINK_STATUS.NOT_LINKED) {
    console.log('\n💡 Link project: lmas mcp link');
  } else if (serverList.enabled === 0) {
    console.log('\n💡 Add server: lmas mcp add context7');
  }
}
