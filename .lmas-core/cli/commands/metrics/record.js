...
function createRecordCommand() {
  ...
  record
    ...
    .option('--quinn', 'Include Oracle metrics', false)
    .option('--quinn-findings <count>', 'Oracle findings count', '0')
    .option('--quinn-categories <list>', 'Oracle categories (comma-separated)')
      ...
      try {
        ...
        if (layerNum === 2) {
          ...
          if (options.verbose) {
            ...
            if (options.quinn) {
              console.log(`Oracle: ${JSON.stringify(result.quinn)}`);
            }
          } else {
