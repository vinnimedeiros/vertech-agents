...
async function detectV2Structure(projectRoot) {
  ...
  if (!fs.existsSync(lmasCoreDir)) {
    return {
      ...
      isV21: false,
      version: null,
      error: 'No .lmas-core directory found',
    };
  }
  ...
  const hasV21Structure = v21Modules.every(module =>
    fs.existsSync(path.join(lmasCoreDir, module)),
  );
  ...
  const hasV20Structure = v20Indicators.some(dir =>
    fs.existsSync(path.join(lmasCoreDir, dir)),
  );
  ...
  return {
    ...
    isV21: false,
    version: null,
    error: 'Unable to detect LMAS version structure',
  };
}

/**
 * Categorize file into target module
 * @param {string} relativePath - Relative path from .lmas-core
...
async function analyzeMigrationPlan(projectRoot, options = {}) {
  ...
  // Get all files
  const allFiles = await getAllFiles(lmasCoreDir);
  ...
  for (const filePath of allFiles) {
    const relativePath = path.relative(lmasCoreDir, filePath);
    ...
    if (module && plan.modules[module]) {
      ...
      fileInfo.targetPath = path.join(lmasCoreDir, module, relativePath);
  ...
  for (const moduleName of Object.keys(plan.modules)) {
    const targetDir = path.join(lmasCoreDir, moduleName);
