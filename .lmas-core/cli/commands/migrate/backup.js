...
/**
 * Create backup of .lmas-core directory
...
async function createBackup(projectRoot, options = {}) {
  ...
  if (!fs.existsSync(lmasCoreDir)) {
    throw new Error('No .lmas-core directory found. Is this an LMAS v2.0 project?');
  }
  ...
  const files = await getAllFiles(lmasCoreDir);
  const manifest = {
    version: '2.0',
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = path.relative(lmasCoreDir, srcFile);
    const destFile = path.join(backupDir, '.lmas-core', relativePath);
...
async function verifyBackup(backupDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Backup manifest not found. Is this a valid LMAS backup?');
  }
  ...
  for (const file of manifest.files) {
    const filePath = file.isConfig
      ...
      : path.join(backupDir, '.lmas-core', file.relativePath);
...
async function findLatestBackup(projectRoot) {
  ...
  const backups = entries
    .filter(entry => entry.isDirectory() && entry.name.startsWith('.lmas-backup-'))
    .map(entry => ({
      name: entry.name,
      path: path.join(projectRoot, entry.name),
      date: entry.name.replace('.lmas-backup-', ''),
    }))
...
async function listBackups(projectRoot) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('.lmas-backup-')) {
