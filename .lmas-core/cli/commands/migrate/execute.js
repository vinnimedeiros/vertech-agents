 ...
 */
async function createModuleDirectories(lmasCoreDir, options = {}) {
 ...
 */
async function migrateModule(moduleData, moduleName, lmasCoreDir, options = {}) {
...
async function executeMigration(plan, options = {}) {
  ...
  if (!dryRun) {
    await createModuleDirectories(plan.lmasCoreDir, { onProgress });
  }
  ...
  if (plan.uncategorized.length > 0 && !dryRun) {
    ...
    for (const file of plan.uncategorized) {
      try {
        const targetPath = path.join(plan.lmasCoreDir, 'core', file.relativePath);
        await copyFileWithMetadata(file.sourcePath, targetPath);
        result.totalFiles++;
        result.totalSize += file.size;
      } catch (error) {
  ...
  if (!dryRun && cleanupOriginals) {
    ...
    for (const [_moduleName, config] of Object.entries(MODULE_MAPPING)) {
      for (const dir of config.directories) {
        const originalDir = path.join(plan.lmasCoreDir, dir);
...
async function saveMigrationState(projectRoot, state) {
  const statePath = path.join(projectRoot, '.lmas-migration-state.json');
