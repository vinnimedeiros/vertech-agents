...
function printHeader(fromVersion, toVersion) {
  console.log('');
  console.log(`🔄 LMAS Migration v${fromVersion} → v${toVersion}`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
}
...
async function runMigration(options) {
  ...
  if (!versionInfo.isV2) {
    ...
    console.error(`❌ ${versionInfo.error || 'Cannot detect LMAS version'}`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (!migrationResult.success) {
      ...
      console.log('  Run: lmas migrate --rollback');
      process.exit(1);
    }
  ...
  try {
    ...
    const importResult = await updateAllImports(lmasCoreDir, plan, { verbose, onProgress });
  ...
  const backupDir = `.lmas-backup-${new Date().toISOString().split('T')[0]}`;
...
async function runRollbackCommand(options) {
  ...
  console.log('');
  console.log('🔙 LMAS Migration Rollback');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
...
async function showBackupStatus(_options) {
  ...
  console.log('');
  console.log('📦 LMAS Migration Backups');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
function createMigrateCommand() {
  ...
  migrate
    .description('Migrate LMAS from v2.0 to v4.0.4 modular structure')
...
Migration Flow:
  Phase 1: Backup
    - Creates .lmas-backup-{date}/ with all files
...
Examples:
  $ lmas migrate                          # Interactive migration
  $ lmas migrate --dry-run                # Preview changes
  $ lmas migrate --verbose                # Detailed output
  $ lmas migrate -y --skip-tests          # Non-interactive, skip tests
  $ lmas migrate --rollback               # Restore from backup
  $ lmas migrate --status                 # Show backups

Rollback:
  If migration fails or you want to revert:
  $ lmas migrate --rollback

  Manual rollback:
  $ cp -r .lmas-backup-{date}/* .lmas-core/
`);
