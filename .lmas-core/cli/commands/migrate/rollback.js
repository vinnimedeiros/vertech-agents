 ...
 */
async function removeV21Structure(lmasCoreDir, options = {}) {
...
async function restoreFromBackup(backup, projectRoot, options = {}) {
  ...
  const backupLmasCoreDir = path.join(backup.path, '.lmas-core');

  // Restore .lmas-core files
  ...
  for (const file of backup.manifest.files) {
    try {
      const sourcePath = file.isConfig
        ...
        : path.join(backupLmasCoreDir, file.relativePath);

      const targetPath = file.isConfig
        ? path.join(projectRoot, file.relativePath)
        : path.join(lmasCoreDir, file.relativePath);
...
async function executeRollback(projectRoot, options = {}) {
  ...
  const lmasCoreDir = path.join(projectRoot, '.lmas-core');
  result.removal = await removeV21Structure(lmasCoreDir, { onProgress });
  ...
  for (const dir of expectedDirs) {
    if (fs.existsSync(path.join(lmasCoreDir, dir))) {
      validCount++;
    }
  }
...
function formatRollbackSummary(result) {
  ...
  lines.push('');
  lines.push('🔙 LMAS Migration Rollback');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
