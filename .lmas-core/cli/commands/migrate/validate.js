 ...
 */
async function validateStructure(lmasCoreDir) {
  const result = {
    valid: true,
    modules: {},
    errors: [],
  ...
  const rootEntries = await fs.promises.readdir(lmasCoreDir, { withFileTypes: true });
...
async function runFullValidation(projectRoot, options = {}) {
  ...
  result.structure = await validateStructure(lmasCoreDir);
  ...
  result.imports = await verifyImports(lmasCoreDir);
...
function generateSummary(migrationResult, validationResult, options = {}) {
  ...
  lines.push('  3. Report issues: github.com/lmas/issues');
  lines.push('');
  lines.push('To rollback: lmas migrate --rollback');
