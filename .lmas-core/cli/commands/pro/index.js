 ...
 * Pro Command Module
 *
 * CLI commands for LMAS Pro license management and feature gating.
 *
 * Subcommands:
 *   lmas pro activate --key <KEY>    Activate a license key
 *   lmas pro status                   Show license status
 *   lmas pro deactivate               Deactivate the current license
 *   lmas pro features                 List all pro features
 *   lmas pro validate                 Force online revalidation
 *   lmas pro setup                    Configure GitHub Packages access (AC-12)
 *
...
// In framework-dev: __dirname = lmas-core/.lmas-core/cli/commands/pro → ../../../../pro/license
// In project-dev: pro is installed via npm as @lmas-fullstack/pro
function resolveLicensePath() {
  ...
  // 2. Try node_modules/@lmas-fullstack/pro/license (project-dev mode)
  try {
    const proPkg = require.resolve('@lmas-fullstack/pro/package.json');
    ...
    // @lmas-fullstack/pro not installed via npm
  }

  // 3. Try project root node_modules (fallback)
  const projectRoot = process.cwd();
  const cwdPath = path.join(projectRoot, 'node_modules', '@lmas-fullstack', 'pro', 'license');
...
function loadLicenseModules() {
  try {
    ...
    console.error('LMAS Pro license module not available.');
    console.error('Install LMAS Pro: npm install @lmas-fullstack/pro');
    process.exit(1);
  }
}

/**
 * Get LMAS Core version from package.json
 */
function getLmasCoreVersion() {
  try {
...
async function activateAction(options) {
  ...
  if (!key) {
    ...
    console.error('Usage: lmas pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
    process.exit(1);
  }
  ...
  console.log('\nActivating LMAS Pro license...');
  console.log(`Key: ${maskKey(key)}`);
  console.log('');

  try {
    const machineId = generateMachineId();
    const lmasCoreVersion = getLmasCoreVersion();

    const result = await licenseApi.activate(key, machineId, lmasCoreVersion);
    ...
    const proSourceDir = path.join(projectRoot, 'node_modules', '@lmas-fullstack', 'pro');
    ...
    if (fs.existsSync(proSourceDir)) {
      ...
      if (scaffoldProContent) {
        ...
        if (scaffoldResult.success) {
          ...
          console.error('Try running "lmas pro activate" again to retry scaffolding.');
        }
        console.log('');
      }
    } else {
      console.log('Note: @lmas-fullstack/pro package not found in node_modules.');
...
function statusAction() {
  ...
  if (!cache) {
    ...
    console.log('  Activate: lmas pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  ...
  if (info && info.inGrace) {
    ...
    console.log('  Please revalidate your license: lmas pro validate');
  }
...
async function deactivateAction(options) {
  ...
  try {
    ...
    console.log('');
    console.log('To reactivate: lmas pro activate --key <KEY>');
    console.log('');
...
// ---------------------------------------------------------------------------
// lmas pro setup (AC-12: Install-gate)
// ---------------------------------------------------------------------------

/**
 * Setup and verify @lmas-fullstack/pro installation.
 *
 * Since @lmas-fullstack/pro is published on the public npm registry,
...
async function setupAction(options) {
  ...
  if (options.verify) {
    // Verify-only mode
    console.log('Verifying @lmas-fullstack/pro installation...\n');

    try {
      const { execSync } = require('child_process');
      const result = execSync('npm ls @lmas-fullstack/pro --json', {
        stdio: 'pipe',
        timeout: 15000,
      });
      const parsed = JSON.parse(result.toString());
      const deps = parsed.dependencies || {};
      if (deps['@lmas-fullstack/pro']) {
        console.log(`✅ @lmas-fullstack/pro@${deps['@lmas-fullstack/pro'].version} is installed`);
      } else {
        console.log('❌ @lmas-fullstack/pro is not installed');
        console.log('');
        console.log('Install with:');
        console.log('  npm install @lmas-fullstack/pro');
      }
    } catch {
      console.log('❌ @lmas-fullstack/pro is not installed');
      console.log('');
  ...
  // Install mode
  console.log('@lmas-fullstack/pro is available on the public npm registry.');
  console.log('No special tokens or configuration needed.\n');

  console.log('Installing @lmas-fullstack/pro...\n');
  ...
  try {
    ...
    console.log('\n✅ @lmas-fullstack/pro installed successfully!');
  } catch (error) {
  ...
  console.log('  lmas pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  console.log('');
  console.log('To check license status:');
  console.log('  lmas pro status');
  console.log('');
...
/**
 * Create the `lmas pro` command with all subcommands.
 * @returns {Command}
 */
function createProCommand() {
  const proCmd = new Command('pro')
    .description('LMAS Pro license management');
  ...
  proCmd
    .command('setup')
    .description('Install and verify @lmas-fullstack/pro')
