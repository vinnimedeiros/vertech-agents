 ...
 *
 * CLI command for validating LMAS-Core installation integrity.
 ...
 *
 * Usage:
 *   lmas validate                    # Validate current installation
 *   lmas validate --repair           # Repair missing/corrupted files
 *   lmas validate --repair --dry-run # Preview repairs without applying
 *   lmas validate --detailed         # Show detailed file list
 *   lmas validate --json             # Output as JSON
 */
...
function createValidateCommand() {
  ...
  validate
    .description('Validate LMAS-Core installation integrity')
...
async function runValidation(options) {
  ...
  // Check if LMAS-Core is installed
    ...
    if (options.json) {
      console.log(
        JSON.stringify(
          {
            status: 'failed',
            error: 'LMAS-Core not found in current directory',
      ...
      console.error(chalk.dim(`Expected at: ${lmasCoreDir}`));
      console.error(chalk.dim('\nRun `npx lmas-core install` to install LMAS-Core'));
    }
  ...
  if (sourceDir) {
    const sourceManifest = path.join(sourceDir, '.lmas-core', 'install-manifest.yaml');
  ...
  if (!sourceDir && options.repair) {
    ...
    const possibleSources = [
      ...
      path.join(projectRoot, 'node_modules/lmas-core'),
      path.join(projectRoot, 'node_modules/lmas-core'),
    ];

    for (const src of possibleSources) {
      if (fs.existsSync(path.join(src, '.lmas-core', 'install-manifest.yaml'))) {
        sourceDir = src;
        break;
      }
    }
