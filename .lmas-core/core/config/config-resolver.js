 ...
 */
const CONFIG_FILES = {
  framework: '.lmas-core/framework-config.yaml',
  project: '.lmas-core/project-config.yaml',
  pro: 'pro/pro-config.yaml',
  local: '.lmas-core/local-config.yaml',
  legacy: '.lmas-core/core-config.yaml',
  user: path.join(os.homedir(), '.lmas', 'user-config.yaml'),
};
...
function loadLayeredConfig(projectRoot, options = {}) {
  ...
  if (options.appDir) {
    const appConfigPath = path.join(options.appDir, 'lmas-app.config.yaml');
  ...
  // L5: User (optional — global user preferences, cross-project, ~/.lmas/user-config.yaml)
...
function loadLegacyConfig(projectRoot) {
  ...
  const suppressDeprecation = process.env.LMAS_SUPPRESS_DEPRECATION === 'true'
    || process.env.LMAS_SUPPRESS_DEPRECATION === '1';

  if (!suppressDeprecation) {
    warnings.push(
      '[DEPRECATION] Monolithic core-config.yaml detected. '
      + 'Run `lmas config migrate` to split into layered config files. '
      + 'Monolithic format will be removed in v4.0.0. '
      + 'Set LMAS_SUPPRESS_DEPRECATION=true to silence this warning.',
    );
  }
...
function getConfigAtLevel(projectRoot, level, options = {}) {
  ...
  switch (level) {
    ...
    case 'app': case '3': case 'L3':
      ...
      relativePath = path.join(options.appDir, 'lmas-app.config.yaml');
      break;
...
/**
 * Ensure the ~/.lmas/ directory exists with secure permissions.
 *
...
/**
 * Set a value in the user config file (~/.lmas/user-config.yaml).
