 ...
 *   - .lmas-core/project-config.yaml (L2) — team-shared project settings
 *   - ~/.lmas/user-config.yaml (L5) — cross-project user preferences
 *
 * Safety:
...
/**
 * Fields that belong in L5 User config (~/.lmas/user-config.yaml).
...
/**
 * Fields that belong in L2 Project config (.lmas-core/project-config.yaml).
...
function isLegacyMode(projectRoot) {
  const legacyPath = path.join(projectRoot, '.lmas-core', 'core-config.yaml');
  const frameworkPath = path.join(projectRoot, '.lmas-core', 'framework-config.yaml');
...
function createBackup(projectRoot) {
  const legacyPath = path.join(projectRoot, '.lmas-core', 'core-config.yaml');
  const backupPath = path.join(projectRoot, '.lmas-core', 'core-config.backup.yaml');
...
/**
 * Ensure the ~/.lmas/ directory exists.
 *
 * @returns {string} Path to ~/.lmas/ directory
 */
function ensureUserConfigDir() {
  const dir = path.join(os.homedir(), '.lmas');
...
/**
 * Write user config to ~/.lmas/user-config.yaml.
...
function writeUserConfig(userFields) {
  ensureUserConfigDir();
  const userConfigPath = path.join(os.homedir(), '.lmas', 'user-config.yaml');
...
/**
 * Write project config to .lmas-core/project-config.yaml.
...
function writeProjectConfig(projectRoot, projectFields) {
  const projectConfigPath = path.join(projectRoot, '.lmas-core', 'project-config.yaml');
