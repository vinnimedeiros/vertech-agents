 ...
 *
 * @module lmas-core/doctor/checks/code-intel
...
async function run(context) {
  const codeIntelDir = path.join(context.projectRoot, '.lmas-core', 'core', 'code-intel');
  ...
  try {
    ...
    if (!available) {
      ...
      const registryPath = path.join(context.projectRoot, '.lmas-core', 'data', 'entity-registry.yaml');
      if (fs.existsSync(registryPath)) {
        ...
        return {
          ...
          fixCommand: 'node .lmas-core/development/scripts/populate-entity-registry.js',
        };
      }
