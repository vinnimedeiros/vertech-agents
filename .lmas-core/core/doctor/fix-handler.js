 ...
 *
 * @module lmas-core/doctor/fix-handler
 * @story INS-4.1
 */
...
const FIX_MAP = {
  ...
  'agent-memory': {
    ...
    async apply(result, { projectRoot }) {
      const agentsDir = path.join(projectRoot, '.lmas-core', 'development', 'agents');
      let created = 0;
  ...
  'claude-md': {
    ...
    async apply() {
      return 'CLAUDE.md regeneration requires npx lmas-core install --force';
    },
  },
  ...
  'settings-json': {
    ...
    async apply(result, { frameworkRoot }) {
      ...
      return 'settings.json regeneration requires npx lmas-core install --force';
    },
  },
};
