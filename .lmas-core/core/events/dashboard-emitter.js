...
const MONITOR_SERVER_URL = process.env.LMAS_MONITOR_URL || 'http://localhost:4001/events';
...
class DashboardEmitter {
  ...
  constructor() {
    ...
    this.fallbackPath = path.join(this.projectRoot, '.lmas', 'dashboard', 'events.jsonl');
  ...
  async emit(type, data = {}) {
    ...
    const event = {
      ...
      lmas_agent: this.currentAgent || undefined,
      lmas_story_id: this.currentStoryId || undefined,
      data,
    };
      ...
      const response = await fetch(MONITOR_SERVER_URL, {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            lmas_agent: event.lmas_agent,
            lmas_story_id: event.lmas_story_id,
          },
        }),
