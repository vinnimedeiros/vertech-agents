 ...
 * Loads project-specific merge rules from .lmas/merge-rules.yaml
 */
class CustomRulesLoader {
  constructor(rootPath = process.cwd()) {
    this.rootPath = rootPath;
    this.rulesPath = path.join(rootPath, '.lmas', 'merge-rules.yaml');
    this.cache = {
      rules: null,
      lastLoad: null,
  ...
  async executeHook(hookName, context = {}) {
    ...
    try {
      execSync(hookCommand, {
        ...
        env: {
          ...process.env,
          LMAS_MERGE_HOOK: hookName,
          LMAS_MERGE_CONTEXT: JSON.stringify(context),
        },
      });
      return true;
    } catch (error) {
...
class SemanticMergeEngine extends EventEmitter {
  constructor(config = {}) {
    ...
    // Storage
    this.storageDir = config.storageDir || path.join(this.rootPath, '.lmas', 'merge');
  }
