...
/**
 * Write HTML graph to .lmas/graph.html and open in default browser.
...
function handleHtmlOutput(graphData, options = {}) {
  const outputDir = path.resolve(process.cwd(), '.lmas');
 ...
 * Writes to .lmas/graph.dot, .lmas/graph.mmd, or .lmas/graph.html.
 * @param {Object} args - Parsed CLI args
...
async function handleWatch(args) {
  ...
  const registryPath = path.resolve(process.cwd(), '.lmas-core/data/entity-registry.yaml');
...
function handleHelp() {
  const usage = `
Usage: lmas graph [command] [options]
...
Examples:
  lmas graph --deps                        Show dependency tree
  lmas graph --deps --format=json          Output as JSON
  lmas graph --deps --format=html          Interactive HTML graph (opens browser)
  lmas graph --deps --watch                Live DOT file for VS Code preview
  lmas graph --deps --watch --format=html  Live HTML with auto-refresh
  lmas graph --deps --watch --format=mermaid  Live Mermaid file
  lmas graph --deps --watch --interval=10  Refresh every 10 seconds
  lmas graph --stats                       Show entity stats and cache metrics
`.trim();
...
async function handleSummary(args) {
  ...
  sections.push('LMAS Graph Dashboard');
