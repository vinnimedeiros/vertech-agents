 ...
 *
 * @module lmas-core/health-check/checks/local/environment-vars
 * @version 1.0.0
...
/**
 * LMAS-specific environment variables
 */
const LMAS_VARS = ['LMAS_DEBUG', 'LMAS_LOG_LEVEL'];
...
class EnvironmentVarsCheck extends BaseCheck {
  ...
  async execute(_context) {
    ...
    // Check LMAS-specific vars (info only)
    for (const varName of LMAS_VARS) {
      if (process.env[varName]) {
        lmasVars.push({ name: varName, value: this.maskValue(process.env[varName]) });
      }
    }
