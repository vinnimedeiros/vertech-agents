 ...
 *
 * Verifies that LMAS framework configuration files are present.
 *
 * @module lmas-core/health-check/checks/project/framework-config
 * @version 1.0.0
...
const REQUIRED_CONFIGS = [
  { path: '.lmas-core', type: 'directory', description: 'LMAS core framework' },
...
const RECOMMENDED_CONFIGS = [
  { path: '.lmas', type: 'directory', description: 'LMAS local configuration' },
...
class FrameworkConfigCheck extends BaseCheck {
  constructor() {
    super({
      ...
      description: 'Verifies LMAS framework configuration is present',
      domain: CheckDomain.PROJECT,
      severity: CheckSeverity.HIGH,
      timeout: 3000,
      cacheable: true,
      healingTier: 0, // Cannot auto-fix framework setup
      tags: ['lmas', 'config', 'framework'],
    });
  }
  ...
  async execute(context) {
    ...
    if (missingRequired.length > 0) {
      ...
      return this.fail(`Missing required framework configuration: ${missing}`, {
        recommendation: 'Run LMAS setup or manually create missing directories',
        details: {
    ...
    if (missingRecommended.length > 0) {
      ...
      return this.warning(`Missing recommended configuration: ${missing}`, {
        recommendation: 'Consider adding recommended configuration for full LMAS functionality',
        details: {
