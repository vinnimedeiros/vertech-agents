 ...
 *
 * @module lmas-core/health-check/checks/services/gemini-cli
 * @version 1.0.0
...
class GeminiCliCheck extends BaseCheck {
  ...
  async execute(context) {
    ...
    try {
      ...
      // Check for LMAS agents
      try {
        const agentsDir = path.join(geminiDir, 'rules', 'LMAS', 'agents');
        const agentFiles = await fs.readdir(agentsDir);
        details.lmasAgents = agentFiles.filter((f) => f.endsWith('.md')).length;
      } catch {
        details.lmasAgents = 0;
      }
    } catch {
      // No project config
    }
    ...
    if (details.projectConfig && details.lmasAgents === 0) {
      warnings.push('No LMAS agents installed for Gemini CLI');
    }
    ...
    if (issues.length > 0) {
      return this.warning(`Gemini CLI needs attention: ${issues.join(', ')}`, {
        recommendation: issues.includes('Not authenticated')
          ...
          : 'Run `npx lmas-core install` and select Gemini CLI',
    ...
    if (warnings.length > 0) {
      return this.warning(`Gemini CLI configuration incomplete: ${warnings.join(', ')}`, {
        recommendation: 'Run `npx lmas-core install` and select Gemini CLI',
    ...
    if (details.lmasAgents > 0) parts.push(`${details.lmasAgents} LMAS agents`);
