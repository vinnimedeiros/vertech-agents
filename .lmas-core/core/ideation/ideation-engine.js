...
class IdeationEngine {
  constructor(config = {}) {
    ...
    // Output paths
    this.outputDir = config.outputDir || path.join(this.rootPath, '.lmas', 'ideation');
  ...
  formatMarkdown(result) {
    ...
    md += '---\n*Generated by LMAS Ideation Engine*\n';
  ...
  checkSyncOperations() {
    ...
    try {
      ...
      const result = execSync(
        `grep -rn "readFileSync\\|writeFileSync\\|existsSync" --include="*.js" --include="*.ts" ${this.rootPath}/src ${this.rootPath}/.lmas-core 2>/dev/null || true`,
        {
          encoding: 'utf8',
...
class CodeQualityAnalyzer {
  ...
  checkLongFunctions() {
    ...
    try {
      const result = execSync(
        `wc -l ${this.rootPath}/src/**/*.js ${this.rootPath}/.lmas-core/**/*.js 2>/dev/null | sort -rn | head -10 || true`,
        {
          encoding: 'utf8',
        },
      );
