 ...
 *
 * Layer Model:
 *   L1 (Framework Core)      — .lmas-core/core/, bin/, constitution.md
 *   L2 (Framework Templates)  — .lmas-core/development/, infrastructure/, product/
 *   L3 (Project Config)       — .lmas-core/data/, MEMORY.md, .claude/, *-config.yaml
...
const LAYER_RULES = [
  // --- L1: Framework Core ---
  { layer: 'L1', test: (p) => p.startsWith('.lmas-core/core/') },
  { layer: 'L1', test: (p) => p.startsWith('bin/') },
  { layer: 'L1', test: (p) => p === '.lmas-core/constitution.md' },

  // --- L3: Project Config (before L2 to catch MEMORY.md inside agents/) ---
  { layer: 'L3', test: (p) => p.startsWith('.lmas-core/data/') },
  ...
  { layer: 'L2', test: (p) => p.startsWith('.lmas-core/development/') },
  { layer: 'L2', test: (p) => p.startsWith('.lmas-core/infrastructure/') },
  { layer: 'L2', test: (p) => p.startsWith('.lmas-core/product/') },
