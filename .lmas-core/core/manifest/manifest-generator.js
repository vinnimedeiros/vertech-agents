 ...
 * Scans the .lmas-core directory structure to build comprehensive manifests.
 *
...
class ManifestGenerator {
  constructor(options = {}) {
    ...
    this.lmasCoreDir = path.join(this.basePath, '.lmas-core');
    this.manifestDir = path.join(this.lmasCoreDir, 'manifests');
    this.version = '2.1.0';
  }
  ...
  async generateAgentsManifest() {
    const agentsDir = path.join(this.lmasCoreDir, 'development', 'agents');
        ...
        try {
          ...
          if (parsed && parsed.agent) {
            ...
            agents.push({
              ...
              status: 'active',
              file_path: `.lmas-core/development/agents/${file}`,
  ...
  async generateWorkersManifest() {
    const registryPath = path.join(this.lmasCoreDir, 'core', 'registry', 'service-registry.json');
  ...
  async generateTasksManifest() {
    const tasksDir = path.join(this.lmasCoreDir, 'development', 'tasks');
      ...
      for (const file of mdFiles) {
        try {
          ...
          tasks.push({
            ...
            file_path: `.lmas-core/development/tasks/${file}`,
            status: 'active',
          });
