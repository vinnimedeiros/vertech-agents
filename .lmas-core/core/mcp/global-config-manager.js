 ...
 *
 * Manages the global MCP configuration at ~/.lmas/mcp/
...
/**
 * Check if global LMAS directory exists
 * @returns {boolean}
 */
function globalDirExists() {
  return fs.existsSync(getGlobalLmasDir());
}
...
function addServer(serverName, serverConfig = null) {
  ...
  if (!config) {
    return { success: false, error: 'Global config not found. Run "lmas mcp setup" first.' };
  }
