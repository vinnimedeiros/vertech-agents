 ...
 * @returns {string} Path to .lmas-core/tools/mcp
 */
function getProjectMcpPath(projectRoot = process.cwd()) {
  return path.join(projectRoot, '.lmas-core', 'tools', 'mcp');
}
...
function createLink(projectRoot = process.cwd(), options = {}) {
  ...
  if (!fs.existsSync(globalPath)) {
    return {
      success: false,
      error: 'Global MCP directory does not exist. Run "lmas mcp setup" first.',
      linkPath,
      globalPath,
    };
  }
