/**
 * Verification Tests for Active LMAS Memory Modules
 *
...
test('merge-rules.yaml exists in .lmas', () => {
  const rulesPath = path.join(process.cwd(), '.lmas', 'merge-rules.yaml');
  const exists = fs.existsSync(rulesPath);

  assertTrue(exists, 'merge-rules.yaml should exist in .lmas');
});
