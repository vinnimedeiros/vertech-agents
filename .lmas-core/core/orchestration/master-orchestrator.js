 ...
 *
 * Features:
 * - AC1: Located in `.lmas-core/core/orchestration/`
...
class MasterOrchestrator extends EventEmitter {
  ...
  async loadState(storyId = null) {
    ...
    const targetPath = targetStoryId
      ? path.join(this.projectRoot, '.lmas', 'master-orchestrator', `${targetStoryId}.json`)
      : this.statePath;
  ...
  async findLatestValidState() {
    const stateDir = path.join(this.projectRoot, '.lmas', 'master-orchestrator');
