 ...
 *
 * Provides async API for spawning LMAS agents in separate terminals
...
async function createContextFile(context, outputDir = os.tmpdir()) {
  ...
  const contextPath = path.join(outputDir, `lmas-context-${Date.now()}.json`);
...
async function spawnInline(agent, task, options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn('bash', [scriptPath, ...args], {
      env: {
        ...process.env,
        LMAS_DEBUG: opts.debug ? 'true' : 'false',
        LMAS_OUTPUT_DIR: opts.outputDir,
        LMAS_INLINE_MODE: 'true', // Signal to pm.sh that we're running inline
      },
...
async function cleanupOldFiles(outputDir = os.tmpdir(), maxAgeMs = 3600000) {
  ...
  try {
    ...
    const lmasFiles = files.filter(
      (f) => f.startsWith('lmas-output-') || f.startsWith('lmas-lock-') || f.startsWith('lmas-context-'),
    );

    for (const file of lmasFiles) {
...
function formatCompatibilityReport(report) {
  const lines = [
    '═══════════════════════════════════════════════════════════════',
    '                 LMAS Terminal Spawner Compatibility Report     ',
    '═══════════════════════════════════════════════════════════════',
    '',
