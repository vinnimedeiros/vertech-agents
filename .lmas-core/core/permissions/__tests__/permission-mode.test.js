...
describe('PermissionMode', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(tempDir, '.lmas'), { recursive: true });
  ...
  describe('load()', () => {
    ...
    it('should load mode from config file', async () => {
      await fs.writeFile(
        path.join(tempDir, '.lmas', 'config.yaml'),
        'permissions:\n  mode: auto\n',
      );
  ...
  describe('setMode()', () => {
    it('should set mode and persist to config', async () => {
      ...
      // Verify persisted
      const configContent = await fs.readFile(path.join(tempDir, '.lmas', 'config.yaml'), 'utf-8');
