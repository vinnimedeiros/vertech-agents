...
function run() {
  ...
  check(1, 'Runtime capability detection module exists', () => {
    return fs.existsSync(path.join(PROJECT_ROOT, '.lmas-core', 'data', 'capability-detection.js'));
  });

  check(2, 'Detection runs at session init (module is importable)', () => {
    const mod = require(path.join(PROJECT_ROOT, '.lmas-core', 'data', 'capability-detection.js'));
    return typeof mod.run === 'function';
  });

  check(3, 'Results stored in .lmas/runtime-capabilities.json', () => {
    const capPath = path.join(PROJECT_ROOT, '.lmas', 'runtime-capabilities.json');
  ...
  check(4, 'Tier 3 tools deferred via best available strategy', () => {
    const caps = JSON.parse(fs.readFileSync(path.join(PROJECT_ROOT, '.lmas', 'runtime-capabilities.json'), 'utf8'));
  ...
  check(7, 'Search accuracy validated (7/7 test queries pass)', () => {
    const { validate } = require(path.join(PROJECT_ROOT, '.lmas-core', 'data', 'tool-search-validation.js'));
    return validate();
  });
  ...
  check(8, 'MCP discipline fallback module exists', () => {
    return fs.existsSync(path.join(PROJECT_ROOT, '.lmas-core', 'data', 'mcp-discipline.js'));
  });

  check(9, 'Essential MCP servers defined in tool-registry.yaml', () => {
    const registry = fs.readFileSync(path.join(PROJECT_ROOT, '.lmas-core', 'data', 'tool-registry.yaml'), 'utf8');
  ...
  check(10, 'Non-essential servers can be re-enabled per-session', () => {
    const mod = fs.readFileSync(path.join(PROJECT_ROOT, '.lmas-core', 'data', 'mcp-discipline.js'), 'utf8');
