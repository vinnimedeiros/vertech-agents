/**
 * LMAS Skill Validator
 * Story GEMINI-INT.5 - Skills Cross-CLI Compatibility
 *
 * Validates that LMAS agent skills work correctly in both
...
class SkillValidator {
  constructor(config = {}) {
    this.agentsDir = config.agentsDir || path.join(process.cwd(), '.lmas-core', 'development', 'agents');
    this.tasksDir = config.tasksDir || path.join(process.cwd(), '.lmas-core', 'development', 'tasks');
    this.errors = [];
    this.warnings = [];
  }
  ...
  generateReport(results) {
    let report = '# LMAS Skills Cross-CLI Compatibility Report\n\n';
