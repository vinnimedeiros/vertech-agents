 ...
 *
 * Downloads squads from the lmas-squads GitHub repository.
...
/**
 * Default registry URL for lmas-squads
 * @constant {string}
 */
const REGISTRY_URL =
  'https://raw.githubusercontent.com/LMAS/lmas-squads/main/registry.json';

/**
 * GitHub API base URL for lmas-squads contents
 * @constant {string}
 */
const GITHUB_API_BASE =
  'https://api.github.com/repos/LMAS/lmas-squads/contents/packages';
...
/**
 * Squad Downloader class for downloading squads from lmas-squads repository
 */
class SquadDownloader {
  ...
  /**
   * Fetch registry from lmas-squads repository
   *
  ...
  async _downloadSquadFiles(squadInfo, targetPath) {
    ...
    if (!Array.isArray(contents)) {
      throw new SquadDownloaderError(
        ...
        'Check if squad exists in lmas-squads repository',
      );
    }
  ...
  _fetch(url, useApi = false) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'LMAS-SquadDownloader/1.0',
        },
      };
