...
/**
 * Default LMAS minimum version
 * @constant {string}
 */
const DEFAULT_LMAS_MIN_VERSION = '2.1.0';
...
class SquadGeneratorError extends Error {
  ...
  static schemaNotFound(schemaPath) {
    return new SquadGeneratorError(
      ...
      'Ensure LMAS is properly installed',
    );
  }
}
...
lmas:
  minVersion: "${config.lmasMinVersion || DEFAULT_LMAS_MIN_VERSION}"
  type: squad
...
${config.description || 'Custom LMAS squad.'}
...
This squad ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'does not inherit'} the core LMAS configuration.
...
> This file ${config.configMode === 'extend' ? 'extends' : config.configMode === 'override' ? 'overrides' : 'is independent of'} the core LMAS coding standards.
...
- Node.js >= 18.x
- LMAS >= ${config.lmasMinVersion || DEFAULT_LMAS_MIN_VERSION}
...
class SquadGenerator {
   ...
   * @param {string} [config.lmasMinVersion] - Minimum LMAS version
  ...
  async generate(config) {
    ...
    const fullConfig = {
      ...
      lmasMinVersion: config.lmasMinVersion || DEFAULT_LMAS_MIN_VERSION,
