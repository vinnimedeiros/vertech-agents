...
class SquadMigrator {
  ...
  async analyze(squadPath) {
    ...
    try {
      ...
      // Check for missing lmas.type
      if (!manifest.lmas?.type) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_LMAS_TYPE',
          message: 'Missing required field: lmas.type',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
      ...
      // Check for missing lmas.minVersion
      if (!manifest.lmas?.minVersion) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_MIN_VERSION',
          message: 'Missing required field: lmas.minVersion',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
          path: 'lmas.minVersion',
          value: '2.1.0',
        });
      }
