      ...
      try {
        ...
        if (!hasYamlFrontmatter && !hasMarkdownHeading) {
          result.warnings.push({
            ...
            file: agentFile,
            message: 'Agent file may not follow LMAS agent definition format',
            suggestion:
  ...
  async validateWorkflows(squadPath) {
    ...
    try {
      ...
      result.warnings.push({
        ...
        suggestion: 'Ensure workflow-validator.js exists in .lmas-core/development/scripts/',
      });
      return result;
    }

    const coreAgentsPath = path.join(process.cwd(), '.lmas-core', 'development', 'agents');
