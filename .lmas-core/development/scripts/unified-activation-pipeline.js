 ...
 * Can be overridden via core-config.yaml pipeline.timeout_ms or LMAS_PIPELINE_TIMEOUT env var.
 * @type {number}
 */
...
const ALL_AGENT_IDS = [
  ...
  'devops', 'lmas-master', 'squad-creator',
];
...
class UnifiedActivationPipeline {
  ...
  async _loadCoreConfig() {
    try {
      const configPath = path.join(this.projectRoot, '.lmas-core', 'core-config.yaml');
   ...
   * Priority: LMAS_PIPELINE_TIMEOUT env > core-config.yaml pipeline.timeout_ms > default
   * @private
  ...
  _resolvePipelineTimeout(coreConfig) {
    ...
    const envTimeout = process.env.LMAS_PIPELINE_TIMEOUT;
    if (envTimeout) {
