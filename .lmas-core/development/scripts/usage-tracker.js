...
/**
 * Usage tracker for LMAS-FULLSTACK framework components
 * Tracks component usage patterns for deprecation warnings and impact analysis
 */
class UsageTracker {
  constructor(options = {}) {
    this.rootPath = options.rootPath || process.cwd();
    this.usageDir = path.join(this.rootPath, '.lmas', 'usage');
  ...
  async getComponentInfo(componentId) {
    ...
    return {
      ...
      type: type,
      name: name,
      file_path: `lmas-core/${type}s/${name}.md`
    };
  }

  async getScanPaths(_options) {
    const defaultPaths = [
      path.join(this.rootPath, 'lmas-core'),
