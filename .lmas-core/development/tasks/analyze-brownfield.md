...
pre-conditions:
  ...
  - [ ] Brownfield Analyzer module is available
    ...
    blocker: true
    validation: |
      Verify .lmas-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js exists
...
- **Tool:** brownfield-analyzer
  ...
  - **Source:** .lmas-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js

- **Tool:** mode-detector
  - **Purpose:** Collect project markers for analysis
  - **Source:** .lmas-core/infrastructure/scripts/documentation-integrity/mode-detector.js
...
- **Script:** brownfield-analyzer.js
  ...
  - **Language:** JavaScript
  - **Location:** .lmas-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer.js
...
Analyze an existing project to understand its structure, tech stack, coding standards, and CI/CD workflows before LMAS integration. This task provides recommendations for safe integration and identifies potential conflicts.
...
```javascript
const { analyzeProject, formatMigrationReport } = require('./.lmas-core/infrastructure/scripts/documentation-integrity/brownfield-analyzer');
...
╠══════════════════════════════════════════════════════════════════════╣
║                                                                      ║
║  • Preserve existing ESLint configuration - LMAS will adapt          ║
║  • Keep existing Prettier settings - LMAS coding-standards.md will d ║
║  • Review existing CI/CD before adding LMAS workflows                ║
║  • LMAS will use existing tsconfig.json settings                     ║
...
|----------|---------|---------|
| `parallel` | Safe to proceed with standard LMAS setup | Use `*setup-project-docs` directly |
...
1. **Review GitHub Workflows:**
   ...
   # Check for potential conflicts with LMAS workflows
...
1. **docs/architecture/ exists:**
   - Decide: Keep existing or merge with LMAS docs
   - Option A: Rename existing to `docs/legacy-architecture/`
   - Option B: Configure LMAS to use alternate path
...
- Run this task BEFORE any LMAS integration
