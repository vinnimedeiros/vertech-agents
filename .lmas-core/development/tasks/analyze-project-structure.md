...
**Purpose:** Analyze an existing LMAS project to understand its structure, services, patterns, and provide recommendations for implementing new features. This is Phase 1 of the Incremental Feature Workflow.
...
- field: project_path
  ...
  required: false
  validation: Valid directory path with .lmas-core/
...
```yaml
pre-conditions:
  - [ ] Project has .lmas-core/ directory
    type: pre-condition
    blocker: true
    validation: |
      Check .lmas-core/ directory exists in project root
    error_message: "Pre-condition failed: Not an LMAS project (.lmas-core/ not found)"
...
acceptance-criteria:
  - [ ] Project structure scanned
    ...
    blocker: true
    validation: |
      Assert .lmas-core/ configuration analyzed
...
1. **Error:** No .lmas-core/ Directory
   - **Cause:** Not an LMAS project
   - **Resolution:** Initialize LMAS first or check directory
...
const scanLocations = {
  lmasCore: '.lmas-core/',
  services: '.lmas-core/infrastructure/services/',
  squads: '.lmas-core/squads/',
  agents: '.lmas-core/development/agents/',
  tasks: '.lmas-core/development/tasks/',
  data: '.lmas-core/data/'
};
```
...
const configPatterns = {
  ...
  configFile: exists('lmas.config.js') || exists('.lmas-core/core-config.yaml'),
...
```
.lmas-core/infrastructure/services/{service-name}/
...
**Generated:** {date}
**Generated By:** @architect (Architect)
**Story:** WIS-15
...
```
.lmas-core/infrastructure/services/{service_name}/
├── README.md
├── index.ts
...
```javascript
const { isCodeIntelAvailable } = require('.lmas-core/core/code-intel');
const { getDependencyGraph, getComplexityAnalysis } = require('.lmas-core/core/code-intel/helpers/planning-helper');
