...
Systematically audit all utilities in `.lmas-core/scripts/` to determine their functional status, classify them as WORKING/FIXABLE/DEPRECATED, and generate actionable recommendations for maintenance and cleanup.
...
for util in .lmas-core/scripts/*.js; do
  name=$(basename $util .js)
  count=$(grep -r "$name" .lmas-core/agents .lmas-core/tasks Squads 2>/dev/null | wc -l)
