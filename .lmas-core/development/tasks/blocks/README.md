...
Blocks are small, focused units of functionality that can be included in multiple tasks. They follow the DRY (Don't Repeat Yourself) principle and provide consistent behavior across the LMAS framework.

## Directory Structure

```
.lmas-core/development/tasks/blocks/
├── README.md                  # This file
...
```javascript
const { loadBlock, executeBlock } = require('.lmas-core/utils/block-loader');
...
| `agent-prompt-template` | Standardized template for spawning LMAS agents | Any skill/task that invokes agents via Task tool |
...
*LMAS Task Blocks System v1.1.0*
