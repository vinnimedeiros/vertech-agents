...
Standardized template for spawning LMAS agents with consistent structure. Ensures all agent invocations follow the same persona loading, context, mission, and output pattern.
...
```markdown
You are {agent_name}, the LMAS {agent_role}. Read your complete agent file at:
{agent_file_path}
...
<!-- Parameters:
  ...
  agent_file_path=.claude/commands/LMAS/agents/architect.md,
...
```javascript
const { loadBlock, renderTemplate } = require('.lmas-core/utils/block-loader');
