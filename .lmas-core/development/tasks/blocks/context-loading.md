...
Load LMAS project context before task execution. Provides git state, gotchas filtered by category, technical preferences, and core configuration.
...
steps:
  ...
  - name: Load Technical Preferences
    ...
    actions:
      - read: .lmas-core/data/technical-preferences.md
    output: preferences

  - name: Load Core Config
    actions:
      - read: .lmas-core/core-config.yaml
...
```javascript
const { loadContext } = require('.lmas-core/utils/context-loader');
...
| File | Purpose |
|------|---------|
| `.lmas/gotchas.json` | Known issues and workarounds |
| `.lmas-core/data/technical-preferences.md` | User-defined patterns |
| `.lmas-core/core-config.yaml` | Project configuration |
