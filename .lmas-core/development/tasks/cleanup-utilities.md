...
const configPath = path.join(__dirname, '../../.lmas-core/core-config.yaml');
...
mkdir -p .backups
cp -r .lmas-core/utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.*utility-name" .lmas-core/agents .lmas-core/tasks .lmas-core/workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name" .lmas-core/agents/ .lmas-core/tasks/ .lmas-core/core-config.yaml

# Count total references
count=$(grep -r "utility-name" .lmas-core/ Squads/ 2>/dev/null | wc -l)
...
```bash
mkdir -p .lmas-core/utils-archive
```

**3.2 Create Archive README**

Create `.lmas-core/utils-archive/ARCHIVE-README.md`:
...
1. **Copy file back**:
   ```bash
   cp .lmas-core/utils-archive/utility-name.js .lmas-core/scripts/
   ```
...
4. **Test thoroughly**:
   ```bash
   node .lmas-core/scripts/test-utilities.js
   ```
...
**Immediate Rollback**:
```bash
rm -rf .lmas-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .lmas-core/utils
```

**Selective Restoration**:
```bash
cp .lmas-core/utils-archive/specific-utility.js .lmas-core/scripts/
```
...
git mv .lmas-core/scripts/utility-name.js .lmas-core/utils-archive/

# Or for multiple files:
git mv .lmas-core/scripts/{lmas-validator-fixed.js,lmas-validator-refactored.js} .lmas-core/utils-archive/
```
...
git mv .lmas-core/scripts/lmas-validator.test.js tests/utils/
```
...
1. lmas-validator-fixed.js - Duplicate of lmas-validator.js
2. lmas-validator-refactored.js - Duplicate of lmas-validator.js
...
...
Update utility count in `.lmas-core/core-config.yaml`:
...
```bash
node .lmas-core/scripts/lmas-validator.js
```
...
for util in $(ls .lmas-core/utils-archive/*.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" .lmas-core/agents .lmas-core/tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
rm -rf .lmas-core/utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS .lmas-core/utils
git checkout .lmas-core/core-config.yaml
```
...
1. `.lmas-core/utils-archive/` - Archive directory with 30 deprecated utilities
2. `.lmas-core/utils-archive/ARCHIVE-README.md` - Archive documentation
3. `.backups/utils.backup-3.18-YYYYMMDD-HHMMSS/` - Timestamped backup
4. Updated `.lmas-core/core-config.yaml` - Corrected utility count
...
- ✅ Framework validation (lmas-validator.js) passes
- ✅ All agents (@dev, @po, @qa) activate successfully
