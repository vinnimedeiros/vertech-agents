...
- Load `lmas-core/core-config.yaml` from the project root
- If the file does not exist, HALT and inform the user: "core-config.yaml not found. This file is required for story creation. You can either: 1) Copy it from GITHUB lmas-core/core-config.yaml and configure it for your project OR 2) Run the LMAS installer against your project to upgrade and add the file automatically. Please add and configure core-config.yaml before proceeding."
...
- **Check code intelligence availability:** Call `isCodeIntelAvailable()` from `.lmas-core/core/code-intel`
- **If available:**
  - Call `detectDuplicateStory(storyDescription)` from `.lmas-core/core/code-intel/helpers/story-helper`
  ...
  - Call `suggestRelevantFiles(storyDescription)` from `.lmas-core/core/code-intel/helpers/story-helper`
...
- Execute `lmas-core/tasks/execute-checklist` `lmas-core/checklists/story-draft-checklist`
- Provide summary to user including:
  ...
  - Checklist Results
  - Next steps: For Complex stories, suggest the user carefully review the story draft and also optionally have the PO run the task `lmas-core/tasks/validate-next-story`
