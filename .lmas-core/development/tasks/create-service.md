...
outputs:
  - name: service_directory
    type: directory
    location: ".lmas-core/infrastructure/services/{service_name}/"
    persistido: true
...
```yaml
pre-conditions:
  - [ ] WIS-10 templates exist at .lmas-core/development/templates/service-template/
    tipo: pre-condition
    blocker: true
  ...
  - [ ] Service name is unique (no existing service with same name)
    tipo: pre-condition
    blocker: true
    validação: Check .lmas-core/infrastructure/services/{name}/ does not exist
...
3. agent-tool - Tool for LMAS agents
...
const { checkBeforeWriting } = require('.lmas-core/core/code-intel/helpers/dev-helper');
...
// Check uniqueness
const targetDir = `.lmas-core/infrastructure/services/${serviceName}/`;
...
```javascript
const templateDir = '.lmas-core/development/templates/service-template/';
const templates = [
  'README.md.hbs',
  'index.ts.hbs',
  'types.ts.hbs',
...
cd .lmas-core/infrastructure/services/{service_name}/
...
============================================
 ...
 Next Steps:
   1. cd .lmas-core/infrastructure/services/{service_name}
