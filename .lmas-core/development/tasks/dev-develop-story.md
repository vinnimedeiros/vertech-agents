...
} = require('./.lmas-core/scripts/decision-recorder');
```
...
2. **Code Intelligence Check (IDS G4)** — Before creating new files or functions:
   - If code intelligence is available (`isCodeIntelAvailable()` from `.lmas-core/core/code-intel`):
     - Call `checkBeforeWriting(fileName, description)` from `.lmas-core/core/code-intel/helpers/dev-helper`
...
7. Execute `.lmas-core/product/checklists/story-dod-checklist.md`
...
- `.lmas-core/product/checklists/story-dod-checklist.md` - Definition of Done checklist
