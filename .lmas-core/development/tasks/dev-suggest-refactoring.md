...
# Suggest Refactoring - LMAS Developer Task
...
```bash
# Analyze single file
*suggest-refactoring lmas-core/scripts/complex-utility.js

# Analyze directory with specific patterns
*suggest-refactoring lmas-core/agents --patterns extract_method,decompose_class --recursive

# Apply high-impact suggestions
*suggest-refactoring lmas-core/utils --threshold 7 --apply-all

# Export suggestions for review
*suggest-refactoring . --recursive --export refactoring-report.json

# Dry run to see changes
*suggest-refactoring lmas-core/agents/developer.md --apply ref-001 --dry-run
```
...
class SuggestRefactoringTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔧 LMAS Refactoring Analysis'));
  ...
  async loadSuggestion(suggestionId) {
    ...
    const cacheFile = path.join(this.rootPath, '.lmas', 'refactoring', `${suggestionId}.json`);
...
6. **Code Intelligence Blast Radius** (if available):
   - Call `assessRefactoringImpact(candidateFiles)` from `.lmas-core/core/code-intel/helpers/dev-helper`
