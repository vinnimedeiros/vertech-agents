...
- campo: gotchas_file
  tipo: string
  destino: .lmas/gotchas.md
  persistido: true

- campo: gotchas_json
  tipo: string
  destino: .lmas/gotchas.json
  persistido: true
...
```yaml
pre-conditions:
  - [ ] Project has .lmas directory initialized
    tipo: pre-condition
    blocker: false
    validação: |
      Check if .lmas/ directory exists, create if not
    error_message: "Creating .lmas/ directory"
...
post-conditions:
  - [ ] gotchas.md file generated/updated
    ...
    blocker: true
    validação: |
      Check .lmas/gotchas.md exists and has content
  ...
  - [ ] gotchas.json schema valid
    ...
    blocker: false
    validação: |
      Validate .lmas/gotchas.json against schema
...
1. **update** (default)
   ...
   - Generates `.lmas/gotchas.md` and `.lmas/gotchas.json`
...
```javascript
const { GotchasDocumenter } = require('.lmas-core/infrastructure/scripts/gotchas-documenter');

const documenter = new GotchasDocumenter(rootPath, {
  outputPath: '.lmas/gotchas.md',
  quiet: false
});
```
...
documenter.mergeWithExisting('.lmas/gotchas.json');
```
...
afterCapture: async (insightsPath) => {
  const { updateGotchas } = require('.lmas-core/infrastructure/scripts/gotchas-documenter');
...
const { getGotchasForSelfCritique } = require('.lmas-core/infrastructure/scripts/gotchas-documenter');
...
node .lmas-core/infrastructure/scripts/gotchas-documenter.js update

# List all gotchas
node .lmas-core/infrastructure/scripts/gotchas-documenter.js list

# List high severity only
node .lmas-core/infrastructure/scripts/gotchas-documenter.js list --severity high

# Search for specific gotchas
node .lmas-core/infrastructure/scripts/gotchas-documenter.js search "zustand"

# List by category
node .lmas-core/infrastructure/scripts/gotchas-documenter.js category TypeScript

# Output as JSON
node .lmas-core/infrastructure/scripts/gotchas-documenter.js list --format json
```
...
3. **Error:** Write permission denied
   - **Cause:** Cannot write to .lmas directory
...
**Script:** gotchas-documenter.js
  ...
  - **Language:** JavaScript
  - **Location:** .lmas-core/infrastructure/scripts/gotchas-documenter.js

---

## Dependencies

- `.lmas-core/development/tasks/capture-session-insights.md` - Provides input insights
- `.lmas-core/product/checklists/self-critique-checklist.md` - Consumes gotchas (AC5)
- `.lmas-core/development/tasks/spec-write-spec.md` - May reference gotchas
