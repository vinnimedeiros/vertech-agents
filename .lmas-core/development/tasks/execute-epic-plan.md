...
- campo: epic_state
  tipo: object
  destino: File system (.lmas/epic-{epicId}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Template workflow must exist
    ...
    blocker: true
    validação: |
      execution.template must resolve to .lmas-core/development/workflows/{template}.yaml
  ...
  - [ ] For action=continue, state file must exist
    ...
    blocker: true
    validação: |
      .lmas/epic-{epicId}-state.yaml must exist with status=active
...
post-conditions:
  - [ ] State file updated with current wave progress
    ...
    blocker: true
    validação: |
      .lmas/epic-{epicId}-state.yaml exists and reflects completed waves/stories
...
4. **Error:** State file corrupted
   ...
   - **Recovery:** Restore from .lmas/epic-{epicId}-state.yaml.bak
...
- Template `epic-orchestration.yaml` exists in `.lmas-core/development/workflows/`
- Inner loop `development-cycle.yaml` exists in `.lmas-core/development/workflows/`
...
**1. Load state** from `.lmas/epic-{epicId}-state.yaml`
...
State preserved at: .lmas/epic-{epicId}-state.yaml
...
PROCEDURE execute_wave(wave, stories, state):
  ...
  Task tool call:
    ...
    subagent_type: "lmas-architect"  # or whatever the gate agent is
    prompt: |
...
This task spawns:
- `@po` (Keymaker) - Story validation, checkpoints
- `@dev` (Neo) - Story implementation (via development-cycle)
- `@architect` (Architect) - Wave gates, final gate
- `@devops` (Operator) - Branch merge, push
- `@qa` (Oracle) - Quality gates (via development-cycle)
