...
pre-conditions:
  - [ ] Project root is a valid codebase
    tipo: pre-condition
    blocker: true
    validação: Check package.json or .lmas-core exists
...
```javascript
const PatternExtractor = require('.lmas-core/infrastructure/scripts/pattern-extractor');
...
Examples:
  ...
  *extract-patterns save                      # Save to .lmas/patterns.md
...
Output File:
  Default location: .lmas/patterns.md
...
Patterns saved to: .lmas/patterns.md
...
post-conditions:
  - [ ] Patterns file created/updated
    tipo: post-condition
    blocker: false
    validação: Check .lmas/patterns.md exists and is valid markdown
...
node .lmas-core/infrastructure/scripts/pattern-extractor.js [command] [options]
...
dependencies:
  modules:
    - .lmas-core/infrastructure/scripts/pattern-extractor.js
  tasks: []
  referenced_by:
    - spec-write.md
tags:
