...
- **topic**: `string`
  ...
  - **Example**: "How can we improve user onboarding for LMAS?"
...
```bash
lmas activate Maestro
lmas brainstorm "How can we improve LMAS user onboarding for non-technical users?"
```
...
```bash
lmas brainstorm "How to reduce API latency in database queries?" \
...
```bash
lmas brainstorm "What should be our open-source expansion strategy for Q1 2026?" \
