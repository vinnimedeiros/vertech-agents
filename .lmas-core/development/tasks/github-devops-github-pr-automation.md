...
if (!context) {
  throw new Error('Unable to detect repository. Run "lmas init" first.');
}
```
...
```javascript
const { generateImpactSummary } = require('.lmas-core/core/code-intel/helpers/devops-helper');
...
### NPM Package with Semantic-Release (lmas-core)
```yaml
github:
  pr:
