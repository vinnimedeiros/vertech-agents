...
- Repository context detected (run `lmas init` if needed)
...
if (!context) {
  ...
  console.error('Run "lmas init" to configure installation mode');
  process.exit(1);
}
...
```javascript
const { assessPrePushImpact, classifyRiskLevel } = require('.lmas-core/core/code-intel/helpers/devops-helper');
