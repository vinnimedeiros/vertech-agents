...
```bash
node bin/lmas-ids.js ids:health
```
...
```bash
node bin/lmas-ids.js ids:health --fix
```
...
- Log all healing actions to `.lmas-core/data/registry-healing-log.jsonl`

### Step 3: JSON Output (Machine-Readable)

```bash
node bin/lmas-ids.js ids:health --json
node bin/lmas-ids.js ids:health --fix --json
```
...
```javascript
const { RegistryHealer } = require('.lmas-core/core/ids/registry-healer');
