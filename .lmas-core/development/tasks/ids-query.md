...
```yaml
pre-conditions:
  - [ ] Entity Registry exists at .lmas-core/data/entity-registry.yaml
    tipo: pre-condition
    blocker: false
    validação: |
...
```bash
# Basic query
node bin/lmas-ids.js ids:query "validate story drafts"

# With JSON output
node bin/lmas-ids.js ids:query "template rendering engine" --json

# Filter by type
node bin/lmas-ids.js ids:query "database migration" --type script

# Filter by category
node bin/lmas-ids.js ids:query "agent persona" --category agents
```
...
const { RegistryLoader } = require(path.resolve('.lmas-core/core/ids/registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve('.lmas-core/core/ids/incremental-decision-engine'));
...
- `lmas ids:create-review` — Review CREATE decisions for 30-day assessment
