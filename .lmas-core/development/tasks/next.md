...
LMAS 4.0.4 runtime-first mode adds deterministic next-step recommendation from
...
pre-conditions:
  ...
  - [ ] Session state exists (optional)
    tipo: pre-condition
    blocker: false
    validação: Check .lmas/session-state.json exists
...
```javascript
const SuggestionEngine = require('.lmas-core/workflow-intelligence/engine/suggestion-engine');
...
```javascript
const formatter = require('.lmas-core/workflow-intelligence/engine/output-formatter');
