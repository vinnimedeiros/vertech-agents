...
- PM tool configured in `.lmas-pm-config.yaml` (or will use local-only mode)
...
```javascript
const { getPMAdapter } = require('../.lmas-core/scripts/pm-adapter-factory');
...
- **Configuration missing**: Inform user to run `lmas init`
...
```javascript
const { syncStoryToPM } = require('../.lmas-core/scripts/story-manager');
