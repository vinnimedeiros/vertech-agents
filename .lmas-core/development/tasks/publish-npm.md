...
5. PUBLISH
   ...
   - Verify: npm view lmas-core@preview version === {new_version}

6. SMOKE TEST
   - Create temp directory
   - npm init -y
   - npm install lmas-core@preview
   - Verify critical files exist in node_modules/lmas-core/:
...
8. REPORT
   ...
   - "Test with: npm install lmas-core@preview"
...
- [ ] `npm view lmas-core@preview` returns new version
...
1. VERIFY PREVIEW EXISTS
   - npm view lmas-core@preview version
   - If no preview: HALT with "No preview version found"

2. CONFIRM WITH USER
   - Display: "Promote v{X.Y.Z} from preview to latest?"
   - Show current latest: npm view lmas-core@latest version
   - Require explicit confirmation

3. PROMOTE
   - npm dist-tag add lmas-core@{version} latest

4. VERIFY
   - npm view lmas-core@latest version === {version}
...
6. REPORT
   ...
   - Release URL
   - "Install with: npm install lmas-core"
```

#### Pre-conditions

- [ ] Preview version exists (`npm view lmas-core@preview`)
...
- [ ] `npm view lmas-core@latest` returns promoted version
...
2. INSTALL
   - npm install lmas-core@{tag} (default: latest)
   - Record: install time, exit code, warnings

3. VERIFY FILES
   - Check node_modules/lmas-core/ contains:
     - .lmas-core/core-config.yaml
     - .lmas-core/constitution.md
     - .lmas-core/development/agents/ (non-empty)
     - .lmas-core/development/tasks/ (non-empty)
     - .claude/hooks/synapse-engine.cjs
     - .claude/hooks/precompact-session-digest.cjs
     - .claude/rules/ (non-empty)
     - bin/lmas.js
     - bin/lmas-minimal.js
   - Check node_modules/lmas-core/ does NOT contain:
     - pro/
     - .env
     - .git/
     - tests/
...
4. TEST INSTALLER (optional, if --full flag)
   - npx lmas-core install --preset minimal
   - Verify: .lmas-core/ created
...
- `--full`: Also test `npx lmas-core install`
...
npm view lmas-core versions --json

# 2. Point latest back to previous version
npm dist-tag add lmas-core@{previous-good-version} latest

# 3. Deprecate broken version with message
npm deprecate lmas-core@{broken-version} "Known issues, use v{previous-good-version}"
...
To fully block a version, use `npm unpublish lmas-core@{version}` (within 72h of publish only).
