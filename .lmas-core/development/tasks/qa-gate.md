...
**ALWAYS** check the `lmas-core/core-config.yaml` for the `qa.qaLocation/gates`
...
2. Call `getBlastRadius(files)` from `.lmas-core/core/code-intel/helpers/qa-helper.js`
...
1. **ALWAYS** create gate file at: `qa.qaLocation/gates` from `lmas-core/core-config.yaml`
