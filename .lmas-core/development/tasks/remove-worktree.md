...
post-conditions:
  - [ ] Worktree directory removed
    tipo: post-condition
    blocker: true
    validação: Directory .lmas/worktrees/{storyId} does not exist
...
Removes an LMAS-managed worktree and its associated branch. Includes safety checks for uncommitted changes and provides options for force removal.
...
Options:
  1. Commit changes first    : cd .lmas/worktrees/{storyId} && git commit
...
You are about to remove:
  • Worktree: .lmas/worktrees/{storyId}
  • Branch:   auto-claude/{storyId}
...
1. `git worktree remove .lmas/worktrees/{storyId}`
...
❌ Cannot remove worktree while inside it.
   ...
   Current directory: .lmas/worktrees/{storyId}
