...
responsavel: Morpheus (Commander)
...
- campo: workflow_state
  tipo: object
  destino: File system (.lmas/{instance-id}-engine-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] For action=continue/status/skip/abort, an active engine state file must exist
    ...
    blocker: true
    validação: |
      Check .lmas/{instance-id}-engine-state.yaml exists with status=active
...
post-conditions:
  ...
  - [ ] State file created with all step outputs
    ...
    blocker: true
    validação: |
      Verify .lmas/{instance-id}-engine-state.yaml exists and contains outputs
...
- **Tool:** workflow-state-manager
  ...
  - **Source:** .lmas-core/development/scripts/workflow-state-manager.js

- **Tool:** workflow-validator
  - **Purpose:** Validate workflow before starting
  - **Source:** .lmas-core/development/scripts/workflow-validator.js
...
- Template: `subagent-step-prompt.md` available at `.lmas-core/development/templates/`
...
- `core` → `.lmas-core/development/workflows/{workflow_name}.yaml`
...
State preserved at: .lmas/{instance_id}-engine-state.yaml
```
...
State saved to: .lmas/{instance_id}-engine-state.yaml
```
...
1. **Load template** from `.lmas-core/development/templates/subagent-step-prompt.md`
...
resolve_agent_path(agent_ref, target_context, squad_name):
  ...
  IF target_context == "hybrid":
    ...
    core_path = ".lmas-core/development/agents/{agent_ref}.md"
...
resolve_task_path(uses_ref, target_context, squad_name):
  IF target_context == "core":
    RETURN ".lmas-core/development/tasks/{uses_ref}.md"
  IF target_context == "squad":
    RETURN "squads/{squad_name}/tasks/{uses_ref}.md"
  IF target_context == "hybrid":
    squad_path = "squads/{squad_name}/tasks/{uses_ref}.md"
    core_path = ".lmas-core/development/tasks/{uses_ref}.md"
    IF squad_path exists → RETURN squad_path
    IF core_path exists → RETURN core_path
...
```yaml
# .lmas/{instance-id}-engine-state.yaml
engine_state:
  workflow_id: {id}
  workflow_name: {name}
