...
- campo: workflow_state
  tipo: object
  destino: File system (.lmas/{instance-id}-state.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] For action=continue/status/skip/abort, an active state file must exist
    ...
    blocker: true
    validação: |
      Check .lmas/{instance-id}-state.yaml exists with status=active
...
- **Tool:** workflow-state-manager
  ...
  - **Implementation:** AI agent reads/writes `.lmas/{instance-id}-state.yaml` files directly
...
- For engine mode: `run-workflow-engine.md` task must exist at `.lmas-core/development/tasks/run-workflow-engine.md`
- State directory `.lmas/` must be writable
...
3. **Create state file** using WorkflowStateManager.createState():
   ...
   - Writes state to `.lmas/{instance-id}-state.yaml`
...
3. **Generate cleanup notes:**
   ...
   State file preserved at: .lmas/{instance-id}-state.yaml
