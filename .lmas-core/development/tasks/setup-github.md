...
Configure complete GitHub DevOps infrastructure for user projects created with LMAS. This task copies GitHub Actions workflows, configures CodeRabbit, sets up branch protection, and manages secrets.
...
- campo: devops_setup_report
  tipo: object
  destino: File system (.lmas/devops-setup-report.yaml)
  persistido: true
...
pre-conditions:
  ...
  - [ ] Not already configured (idempotency check)
    ...
    blocker: false
    validação: |
      Check .lmas/devops-setup-report.yaml existence
...
post-conditions:
  ...
  - [ ] DevOps setup report generated
    ...
    blocker: false
    validação: |
      Test-Path ".lmas/devops-setup-report.yaml"
...
acceptance-criteria:
  ...
  - [ ] Setup report documents all configurations
    ...
    blocker: true
    validação: |
      .lmas/devops-setup-report.yaml contains all setup details
...
# Check idempotency
if (Test-Path ".lmas/devops-setup-report.yaml") {
...
$ciTemplate = Get-Content ".lmas-core/infrastructure/templates/github-workflows/ci.yml.template" -Raw
...
Copy-Item ".lmas-core/infrastructure/templates/github-workflows/pr-automation.yml.template" `
  -Destination ".github/workflows/pr-automation.yml"
Write-Host "✅ Installed pr-automation.yml"

# Copy release.yml
Copy-Item ".lmas-core/infrastructure/templates/github-workflows/release.yml.template" `
...
$coderabbitConfig = Get-Content ".lmas-core/infrastructure/templates/coderabbit.yaml.template" -Raw
...
New-Item -ItemType Directory -Path ".lmas" -Force | Out-Null

$report | Out-File -FilePath ".lmas/devops-setup-report.yaml" -Encoding utf8

Write-Host "✅ Setup report saved to .lmas/devops-setup-report.yaml"
```
...
║     git add .github/ .coderabbit.yaml .lmas/                              ║
║     git commit -m "chore: add DevOps configuration [Story 5.10]"          ║
║     git push                                                               ║
║                                                                            ║
║  Report saved: .lmas/devops-setup-report.yaml                             ║
║                                                                            ║
╚═══════════════════════════════════════════════════════════════════════════╝

— Operator, DevOps configured with confidence 🚀
```
