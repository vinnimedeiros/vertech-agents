...
Configure Docker MCP Toolkit as the primary MCP infrastructure for LMAS, using **HTTP transport** instead of stdio to avoid timeout issues during gateway initialization.
...
Configure Docker MCP Toolkit as the primary MCP infrastructure for LMAS, replacing 1MCP with the containerized gateway approach. This enables:
