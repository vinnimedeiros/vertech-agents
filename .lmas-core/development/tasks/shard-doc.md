...
[[LLM: First, check if markdownExploder is set to true in .lmas-core/core-config.yaml. If it is, attempt to run the command: `md-tree explode {input file} {output path}`.
...
2. Or set markdownExploder to false in .lmas-core/core-config.yaml

**IMPORTANT: STOP HERE - do not proceed with manual sharding until one of the above actions is taken.**"

If markdownExploder is set to false, inform the user: "The markdownExploder setting is currently false. For better performance and reliability, you should:

1. Set markdownExploder to true in .lmas-core/core-config.yaml
