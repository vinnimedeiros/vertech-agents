...
Read the current permission mode from `.lmas/config.yaml` under `permissions.mode`.
...
Update `.lmas/config.yaml` with the new `permissions.mode` value.
The `PermissionMode._saveToConfig()` method handles this, including creating the `.lmas/` directory if needed.
...
```javascript
const { cycleMode } = require('./.lmas-core/core/permissions');
...
- If `.lmas/config.yaml` cannot be read, start from default `ask` mode
