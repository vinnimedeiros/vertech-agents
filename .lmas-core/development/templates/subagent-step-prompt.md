...
> Each variable is replaced at runtime by the orchestrator (lmas-master) before spawning the subagent via the Task tool.
...
|----------|--------|-------------|
| `{{AGENT_NAME}}` | Agent file → `agent.name` | Agent's display name (e.g., "Morpheus", "Luan") |
...
|---------|-----------|-----------|-----------|
| `core` | `.lmas-core/development/agents/{agent}.md` | `.lmas-core/development/tasks/{uses}.md` | `.lmas-core/data/{file}` |
...
2. If not found, check `.lmas-core/development/agents/{agent}.md`
...
- The orchestrator (lmas-master) builds the prompt by reading files and replacing variables
