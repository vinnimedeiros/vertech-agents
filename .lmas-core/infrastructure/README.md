...
Base layer of the LMAS modular architecture. Contains tools, integrations, scripts, and PM adapters.
...
- `ComponentGenerator` - LMAS component generation
- `BatchCreator` - Batch operations

### Validation
- `Luan - MultiAgentes ScrumValidator` - LMAS component validation
...
const {
  ...
  TemplateEngine,
  resolveTool
} = require('.lmas-core/infrastructure');

// Or import directly from scripts
const GitWrapper = require('.lmas-core/infrastructure/scripts/git-wrapper');
```

## Tool Resolution

```javascript
const { resolveTool } = require('.lmas-core/infrastructure');
...
```javascript
const { getPMAdapter, isPMToolConfigured } = require('.lmas-core/infrastructure');
