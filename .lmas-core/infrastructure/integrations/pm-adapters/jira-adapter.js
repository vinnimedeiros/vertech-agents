...
class JiraAdapter extends PMAdapter {
   ...
   * @param {string} config.project_key - Project key (e.g., "LMAS")
   */
  constructor(config) {
    super(config);
  ...
  async pullStory(storyId) {
    try {
      ...
      // Map Jira status to LMAS status
      const statusMapping = {
        'To Do': 'Draft',
  ...
  async updateStatus(storyId, status) {
    try {
      ...
      // Map LMAS status to Jira transition
   ...
   * Get Jira issue
   * @private
   * @param {string} issueKey - Issue key (e.g., "LMAS-123")
