...
function getDefaultOptions() {
  ...
  const envLocalDir = process.env.LMAS_CODEX_LOCAL_SKILLS_DIR;
  const envGlobalDir = process.env.LMAS_CODEX_GLOBAL_SKILLS_DIR;
  return {
    projectRoot,
    sourceDir: path.join(projectRoot, '.lmas-core', 'development', 'agents'),
...
function getSkillId(agentId) {
  ...
  if (id.startsWith('lmas-')) return id;
  return `lmas-${id}`;
}
...
function buildSkillContent(agentData) {
  ...
  const title = agent.title || 'LMAS Agent';
...
# LMAS ${title} Activator

## When To Use
${whenToUse}

## Activation Protocol
1. Load \`.lmas-core/development/agents/${agentData.filename}\` as source of truth (fallback: \`.codex/agents/${agentData.filename}\`).
2. Adopt this agent persona and command system.
3. Generate greeting via \`node .lmas-core/development/scripts/generate-greeting.js ${agentData.id}\` and show it first.
