...
// Purpose: Collect tool usage data per session, store in .lmas/analytics/,
//          enforce data governance (sanitization, retention, schema).
//
// Usage:
//   node .lmas-core/infrastructure/scripts/collect-tool-usage.js [options]
//
// Options:
...
// --- Configuration ---
const ANALYTICS_DIR = path.resolve(process.cwd(), '.lmas', 'analytics');
...
function loadUsageData() {
  ...
  return {
    ...
    description: 'Tool usage tracking data for LMAS token optimization (TOK-5)',
    sessions: []
  };
}
