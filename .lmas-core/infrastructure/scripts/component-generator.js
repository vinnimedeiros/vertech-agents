...
class ComponentGenerator {
  constructor(options = {}) {
    ...
    this.templatesPath = path.join(this.rootPath, 'lmas-core', 'templates');
    this.outputPaths = {
      agent: path.join(this.rootPath, 'lmas-core', 'agents'),
      task: path.join(this.rootPath, 'lmas-core', 'tasks'),
      workflow: path.join(this.rootPath, 'lmas-core', 'workflows'),
    };
  ...
  async createAgentCommandTasks(variables, options) {
    ...
    for (const command of variables.COMMANDS) {
      const taskId = this.commandToTaskId(command.COMMAND_NAME);
      const taskPath = path.join(this.rootPath, 'lmas-core', 'tasks', `${taskId}.md`);
