 ...
 * Used for brownfield mode installation to adapt LMAS to existing projects.
 *
  ...
  if (hasDocDir) {
    ...
    if (fs.existsSync(archDir)) {
      analysis.conflicts.push(
        'docs/architecture/ already exists - LMAS docs may need different location',
      );
    }
  }
}
...
function generateRecommendations(analysis) {
  ...
  if (analysis.linting !== 'none') {
    analysis.recommendations.push(
      `Preserve existing ${analysis.linting} configuration - LMAS will adapt`,
    );
  } else {
  ...
  if (analysis.formatting !== 'none') {
    analysis.recommendations.push(
      `Keep existing ${analysis.formatting} settings - LMAS coding-standards.md will document them`,
    );
  }

  // Workflow recommendations
  if (analysis.hasExistingWorkflows) {
    analysis.recommendations.push('Review existing CI/CD before adding LMAS workflows');
    analysis.mergeStrategy = 'manual';
  } else {
    analysis.recommendations.push('Use *setup-github to add LMAS standard workflows');
    analysis.mergeStrategy = 'parallel';
  }

  // TypeScript recommendations
  if (analysis.configs.tsconfig) {
    analysis.recommendations.push('LMAS will use existing tsconfig.json settings');
  }
  ...
  if (analysis.frameworks.includes('NestJS')) {
    analysis.recommendations.push('NestJS detected - LMAS will adapt to module structure');
  }
}
