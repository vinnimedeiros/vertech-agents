...
const GitignoreTemplates = {
  LMAS_BASE: 'gitignore-lmas-base.tmpl',
...
function getTemplatesForStacks(techStacks) {
  const templates = [GitignoreTemplates.LMAS_BASE];
...
function generateGitignore(markers, options = {}) {
  ...
  sections.push('# Generated by LMAS Documentation Integrity System');
...
/**
 * Merges LMAS ignores with existing .gitignore
 *
...
function mergeGitignore(existingContent, _options = {}) {
  // Check if LMAS section already exists
  if (existingContent.includes('LMAS Integration Section')) {
    console.log('LMAS section already exists in .gitignore, skipping merge');
...
# LMAS Local Configuration
.lmas-core/local/
.lmas-core/*.local.yaml
.lmas-core/logs/
.lmas-core/cache/
...
/**
 * Checks if a .gitignore file has LMAS integration
 *
 * @param {string} targetDir - Target directory
 * @returns {boolean} True if LMAS section exists
 */
function hasLmasIntegration(targetDir) {
  const gitignorePath = path.join(targetDir, '.gitignore');
  ...
  return content.includes('LMAS Integration Section') || content.includes('.lmas-core/');
}
...
function parseGitignore(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('LMAS Integration Section')) {
      if (lmasStart === -1) {
        lmasStart = i;
      }
    }

    if (line.includes('End of LMAS Integration Section')) {
      lmasEnd = i;
    }

    if (line.startsWith('#') && !line.includes('LMAS')) {
  ...
  if (lmasStart !== -1) {
    sections.lmasSection = {
      start: lmasStart,
      end: lmasEnd !== -1 ? lmasEnd : lines.length - 1,
    };
  }
