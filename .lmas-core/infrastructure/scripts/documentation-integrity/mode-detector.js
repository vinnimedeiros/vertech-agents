 ...
 *
 * Detects installation mode for LMAS projects with three-mode support:
...
/**
 * Installation modes supported by LMAS
 * @enum {string}
 */
...
const ModeDescriptions = {
  [InstallationMode.FRAMEWORK_DEV]: {
    ...
    hint: 'Developing lmas-core itself - uses framework standards, skips project setup',
    description: 'For LMAS contributors working on the framework',
  },
  [InstallationMode.GREENFIELD]: {
    label: '🆕 New Project (Greenfield)',
    hint: 'Start a fresh project with LMAS - generates project docs, config, and infrastructure',
    description: 'Empty directory setup with full scaffolding',
  },
  [InstallationMode.BROWNFIELD]: {
    label: '📂 Existing Project (Brownfield)',
    hint: 'Add LMAS to existing project - analyzes current structure and adapts',
 ...
 * 1. FRAMEWORK_DEV - .lmas-core/ exists AND is lmas-core repo
...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  // Priority 1: Check for LMAS framework development
  if (markers.hasLmasCore && markers.isLmasCoreRepo) {
    return {
      mode: InstallationMode.FRAMEWORK_DEV,
      legacyType: LegacyProjectType.EXISTING_LMAS,
      confidence: 100,
      reason: 'Detected lmas-core repository with .lmas-core directory',
      markers,
    };
  }

  // Priority 2: Check for existing LMAS installation (non-framework)
  if (markers.hasLmasCore && !markers.isLmasCoreRepo) {
    return {
      mode: InstallationMode.BROWNFIELD,
      legacyType: LegacyProjectType.EXISTING_LMAS,
      confidence: 95,
      reason: 'LMAS already installed in user project - treating as brownfield update',
      markers,
    };
  }
...
function collectMarkers(targetDir) {
  ...
  return {
    // LMAS markers
    hasLmasCore: fs.existsSync(path.join(targetDir, '.lmas-core')),
    isLmasCoreRepo: isLmasCoreRepository(targetDir),
...
/**
 * Checks if the target directory is the lmas-core repository itself
 *
 * @param {string} targetDir - Directory to check
 * @returns {boolean} True if this is the lmas-core repository
 */
function isLmasCoreRepository(targetDir) {
  ...
  try {
    ...
    // Primary check: explicit lmas-core package names
    if (packageJson.name === '@lmas/core' || packageJson.name === 'lmas-core') {
      return true;
    }

    // Secondary check: workspaces pattern + lmas-specific marker file
    // This prevents false positives for generic monorepos
    const hasLmasMarker = fs.existsSync(path.join(targetDir, '.lmas-core', 'infrastructure'));
    const hasWorkspaces =
      Array.isArray(packageJson.workspaces) && packageJson.workspaces.includes('packages/*');

    return hasWorkspaces && hasLmasMarker;
  } catch (error) {
    // Log error for debugging but don't throw - return false for safety
    if (process.env.LMAS_DEBUG) {
      console.warn(`[mode-detector] Error checking lmas-core repository: ${error.message}`);
    }
    return false;
  }
}
 ...
 * for EXISTING_LMAS legacy type (both use EXISTING_LMAS but with different modes).
 * For accurate mode detection, use the mode property from detectInstallationMode().
 *
 * @param {string} legacyType - Legacy project type (EXISTING_LMAS, GREENFIELD, etc.)
 * @param {Object} [context] - Optional context for disambiguation
 * @param {boolean} [context.isLmasCoreRepo] - True if this is the lmas-core repository
 * @returns {string} New installation mode
 */
function mapLegacyTypeToMode(legacyType, context = {}) {
  // EXISTING_LMAS is ambiguous: it can be FRAMEWORK_DEV (lmas-core repo)
  // or BROWNFIELD (user project with LMAS installed)
  if (legacyType === LegacyProjectType.EXISTING_LMAS) {
    // Use context to disambiguate if provided
    if (context.isLmasCoreRepo === true) {
      return InstallationMode.FRAMEWORK_DEV;
    }
    if (context.isLmasCoreRepo === false) {
...
function validateModeSelection(selectedMode, detected) {
  ...
  if (selectedMode !== detected.mode) {
    ...
    if (selectedMode === InstallationMode.FRAMEWORK_DEV && !detected.markers.isLmasCoreRepo) {
      result.warnings.push(
        'Selected framework-dev but this does not appear to be the lmas-core repository.',
      );
    }
