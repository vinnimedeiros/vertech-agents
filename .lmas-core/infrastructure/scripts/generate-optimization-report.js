...
// Layer: L2 (.lmas-core/infrastructure/scripts/)
// Purpose:
...
//
// Usage:
//   node .lmas-core/infrastructure/scripts/generate-optimization-report.js [options]
//
// Options:
...
const ANALYTICS_DIR = path.resolve(ROOT, '.lmas', 'analytics');
...
const TOOL_REGISTRY_FILE = path.resolve(ROOT, '.lmas-core', 'data', 'tool-registry.yaml');
...
function compareBaseline(baseline, usageData, registry) {
  ...
  for (const [wfName, wfData] of Object.entries(baselineWorkflows)) {
    ...
    const baselineOverheadPct = baseline.comparison?.lmasActual?.overheadPercentOfTypicalSession?.[wfName] || 0;
