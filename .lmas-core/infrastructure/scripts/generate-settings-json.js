...
const CORE_CONFIG_FILE = '.lmas-core/core-config.yaml';
 ...
 * - .lmas-core/core/** gets expanded one level deep
...
function expandProtectedPaths(protectedPaths, exceptions, projectRoot) {
  ...
  for (const globPath of protectedPaths) {
    if (globPath === '.lmas-core/core/**') {
      const { dirs, files } = expandOneLevel('.lmas-core/core', projectRoot);
