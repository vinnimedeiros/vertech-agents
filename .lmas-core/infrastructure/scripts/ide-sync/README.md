...
Automatically synchronizes LMAS agent definitions to IDE command files.

## Overview

IDE Sync keeps agent definitions in `.lmas-core/development/agents/` synchronized with IDE-specific command files in:

- `.claude/commands/LMAS/agents/` (Claude Code)
- `.codex/agents/` (Codex CLI support files)
- `.gemini/rules/LMAS/agents/` (Gemini CLI)
...
npm run sync:ide
# or
node .lmas-core/infrastructure/scripts/ide-sync/index.js sync
```
...
# or
node .lmas-core/infrastructure/scripts/ide-sync/index.js validate
```
...
# or
node .lmas-core/infrastructure/scripts/ide-sync/index.js validate --strict
```
...
Configure in `.lmas-core/core-config.yaml`:
...
```markdown
# Agent Redirect: lmas-developer -> lmas-master

This agent has been renamed. Use `lmas-master` instead.
```

## File Structure

```
.lmas-core/infrastructure/scripts/ide-sync/
