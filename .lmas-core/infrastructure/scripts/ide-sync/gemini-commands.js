...
const FALLBACK_DESCRIPTION = 'Agente especializado LMAS';
...
function commandSlugForAgent(agentId) {
  if (agentId.startsWith('lmas-')) {
    return agentId.replace(/^lmas-/, '');
  }
  return agentId;
}

function menuCommandName(agentId) {
  return `/lmas-${commandSlugForAgent(agentId)}`;
}
...
function buildAgentDescription(agent) {
  ...
  return `Ativar agente LMAS ${agent.id}`;
}
...
function buildAgentCommandPrompt(agentId) {
  return [
    ...
    `3. Renderize o greeting via: node .lmas-core/development/scripts/generate-greeting.js ${agentId}`,
...
function buildMenuPrompt(commandFiles) {
  const lines = [
    'Você está no launcher LMAS para Gemini.',
    '',
...
function buildMenuCommandFile(commandFiles) {
  const content = [
    'description = "Menu rápido LMAS (lista agentes e orienta qual ativar)"',
    'prompt = """',
