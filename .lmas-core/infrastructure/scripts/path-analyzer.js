...
const REFERENCE_PATTERNS = [
  // Markdown code refs to .lmas-core
  { pattern: /`\.lmas-core\/[^`]+`/g, type: 'code-ref' },
  // Task references
...
const ASSET_DIRS = {
  task: '.lmas-core/development/tasks',
  template: '.lmas-core/product/templates',
  checklist: '.lmas-core/product/checklists',
  script: '.lmas-core/infrastructure/scripts',
  data: '.lmas-core/development/data',
  schema: '.lmas-core/schemas',
  agent: '.lmas-core/development/agents',
};
...
async function findFilesToAnalyze(rootPath) {
  const files = [];
  const dirsToScan = [
    { dir: '.lmas-core/development/agents', type: 'agent' },
    { dir: '.lmas-core/development/tasks', type: 'task' },
    { dir: '.lmas-core/product/templates', type: 'template' },
    { dir: '.lmas-core/product/checklists', type: 'checklist' },
  ];
...
function resolveReference(ref, rootPath) {
  ...
  if (ref.reference.startsWith('.lmas-core/')) {
...
function formatConsoleOutput(report, verbose = false) {
  ...
  lines.push('');
  lines.push('═══════════════════════════════════════════════════════════');
  lines.push('  LMAS Path Analysis Report');
...
Options:
  ...
  while (rootPath !== '/') {
    try {
      await fs.access(path.join(rootPath, '.lmas-core'));
      break;
    } catch {
  ...
  if (rootPath === '/') {
    console.error('Error: Could not find .lmas-core directory. Run from project root.');
    process.exit(1);
  }
