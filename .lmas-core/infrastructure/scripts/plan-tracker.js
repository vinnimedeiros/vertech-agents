 ...
 *
 * Features:
 * - AC1: Located in `.lmas-core/infrastructure/scripts/`
 ...
 * - AC7: Integrates with `.lmas/dashboard/status.json`
 *
...
class PlanTracker {
  ...
  _findPlanPath() {
    const searchPaths = [
      ...
      // .lmas plans location
      path.join(this.rootPath, '.lmas/plans', this.storyId, CONFIG.implementationFile),
      // Epic-based location (e.g., lmas-core-ade/story-4.6)
   ...
   *
   * Updates `.lmas/dashboard/status.json` for dashboard integration.
   * Also updates legacy `.lmas/status.json` for backwards compatibility.
   *
...
async function main() {
  ...
  if (args.length < 1 || args.includes('--help') || args.includes('-h')) {
    console.log(`
\u{1F4CA} Plan Progress Tracker - LMAS Execution Engine (Story 4.6)
