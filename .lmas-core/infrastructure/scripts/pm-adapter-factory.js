 ...
 * Automatically selects the correct adapter based on .lmas-pm-config.yaml.
 *
 * Falls back to LocalAdapter if no PM tool is configured, ensuring
 * LMAS works 100% standalone without external dependencies.
 *
 ...
 *
 * Reads .lmas-pm-config.yaml from project root and instantiates
 * the appropriate adapter. Caches the adapter for subsequent calls.
 *
 * Falls back to LocalAdapter if:
 * - No .lmas-pm-config.yaml file exists
...
function getPMAdapter() {
  ...
  const configPath = path.join(process.cwd(), '.lmas-pm-config.yaml');

  // If no PM config file, use local-only adapter
  if (!fs.existsSync(configPath)) {
    console.log('ℹ️  No PM tool configured - using local-only mode');
    console.log('   To configure a PM tool, run: lmas init');
 ...
 *
 * Reads .lmas-pm-config.yaml and returns the tool type.
