 ...
 * for display in agent greetings across all 11 LMAS agents.
 *
...
class ProjectStatusLoader {
  ...
  loadConfig() {
    try {
      const configPath = path.join(this.rootPath, '.lmas-core', 'core-config.yaml');
  ...
  _resolveCacheFilePath() {
    try {
      ...
      if (normalizedGitDir !== normalizedCommonDir) {
        ...
        return path.join(this.rootPath, '.lmas', `project-status-${worktreeHash}.yaml`);
      }
    } catch (error) {
      // Not a git repo or git not available - use default path
    }

    return path.join(this.rootPath, '.lmas', 'project-status.yaml');
  }
 ...
 * @property {string} path - Relative path to worktree (.lmas/worktrees/{story-id})
