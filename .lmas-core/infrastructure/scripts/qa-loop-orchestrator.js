...
const CONFIG = {
  ...
  // Workflow definition
  workflowPath: '.lmas-core/development/workflows/qa-loop.yaml',
  ...
  persistenceIndexPath: '.lmas/qa-loops-index.json', // Track all active loops
};
...
class QALoopOrchestrator {
  ...
  _loadMaxIterations() {
    ...
    const configPaths = [
      path.join(this.rootPath, '.lmas/config.yaml'),
      path.join(this.rootPath, '.lmas/config.yml'),
      path.join(this.rootPath, 'lmas.config.js'),
    ];
...
function printHelp() {
  console.log(`
📊 QA Loop Orchestrator - LMAS QA Evolution (Story 6.5)
