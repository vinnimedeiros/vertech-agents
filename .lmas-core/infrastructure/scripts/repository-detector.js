 ...
 * @property {string} frameworkLocation - Path to LMAS framework files
...
function detectRepositoryContext() {
  ...
  // Detect if we're in lmas-core repo itself
  const isFrameworkRepo =
    packageJson.name === '@lmas/fullstack' ||
    packageJson.name === 'lmas-core' ||
    remoteUrl.includes('lmas-core');

  // Load installation config if exists
  let installConfig = null;
  const configPath = path.join(cwd, '.lmas-installation-config.yaml');
  ...
  return {
    ...
    projectRoot: cwd,
    frameworkLocation: isFrameworkRepo ? cwd : path.join(cwd, 'node_modules/@lmas/fullstack'),
