...
function validateClaudeIntegration(options = {}) {
  ...
  const agentsDir = options.agentsDir || path.join(projectRoot, '.claude', 'commands', 'LMAS', 'agents');
  const hooksDir = options.hooksDir || path.join(projectRoot, '.claude', 'hooks');
  const sourceAgentsDir =
    options.sourceAgentsDir || path.join(projectRoot, '.lmas-core', 'development', 'agents');
