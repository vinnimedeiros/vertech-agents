...
function getDefaultOptions() {
  ...
  return {
    ...
    agentsDir: path.join(projectRoot, '.gemini', 'rules', 'LMAS', 'agents'),
    commandsDir: path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: path.join(projectRoot, 'packages', 'gemini-lmas-extension'),
    sourceAgentsDir: path.join(projectRoot, '.lmas-core', 'development', 'agents'),
    quiet: false,
    json: false,
  };
}
...
function validateGeminiIntegration(options = {}) {
  ...
  const resolved = {
    ...
    agentsDir: options.agentsDir || path.join(projectRoot, '.gemini', 'rules', 'LMAS', 'agents'),
    commandsDir: options.commandsDir || path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: options.extensionDir || path.join(projectRoot, 'packages', 'gemini-lmas-extension'),
    sourceAgentsDir: options.sourceAgentsDir || path.join(projectRoot, '.lmas-core', 'development', 'agents'),
  };
  const errors = [];
  const warnings = [];
  ...
  if (!commandFiles.includes('lmas-menu.toml')) {
    errors.push(`Missing Gemini command file: ${path.relative(resolved.projectRoot, path.join(resolved.commandsDir, 'lmas-menu.toml'))}`);
  }
  ...
  const requiredExtensionFiles = [
    'extension.json',
    'README.md',
    path.join('commands', 'lmas-status.js'),
    path.join('commands', 'lmas-agents.js'),
    path.join('commands', 'lmas-validate.js'),
