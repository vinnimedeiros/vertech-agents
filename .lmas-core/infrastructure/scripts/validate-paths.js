...
function getDefaultOptions() {
  ...
  return {
    ...
    requiredFiles: [
      ...
      path.join(projectRoot, '.lmas-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'),
    ],
    quiet: false,
    json: false,
  };
...
function listSkillFiles(skillsDir) {
  ...
  return fs.readdirSync(skillsDir, { withFileTypes: true })
    .filter(entry => entry.isDirectory() && entry.name.startsWith('lmas-'))
...
function validateSkillPathConventions(content, filePath) {
  const errors = [];
  if (!content.includes('.lmas-core/development/agents/')) {
    errors.push(`${filePath} missing canonical source path ".lmas-core/development/agents/"`);
  }
  if (!content.includes('.lmas-core/development/scripts/generate-greeting.js')) {
