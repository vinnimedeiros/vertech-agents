...
LMAS Presets are pre-configured bundles that provide ready-to-use configurations for specific use cases, project types, or team structures. Presets simplify the initial setup process and ensure consistent configuration across projects.
...
| ------------ | ----------------------- | ------------------ | ----------------- |
| `minimal`    | Bare minimum LMAS setup | dev                | Quick prototyping |
...
lmas install --preset standard

# Install with multiple presets (layered)
lmas install --preset standard --preset frontend

# Install custom preset from directory
lmas install --preset ./presets/my-preset
```
...
lmas presets show standard

# Compare two presets
lmas presets diff minimal standard
```
...
lmas presets export --name my-project-preset --output ./presets/

# Export with description
lmas presets export --name my-preset --description "Project template" --output ./presets/
```
...
```bash
lmas install --preset standard --preset frontend --preset small-team
```
...
2. **Review before install** - Use `lmas presets show` to understand what you're getting
3. **Export before modifying** - Save your config as a preset before major changes
4. **Keep presets updated** - Check for preset updates when upgrading LMAS
