...
|---|---------|----------|-------|-------|-----------|----------|-----------|----------------|
| 1 | example-command | Framework | lmas-master | HIGH (15) | create-doc.md | ✅ KEEP | Core meta-framework capability | N/A |
| 2 | party-mode | Utility | lmas-master | ZERO (0) | N/A | ❌ REMOVE | Novelty feature, unused | Removed in v3.0 |
...
| 5 | create-prd | Docs | lmas-master | MED (7) | create-doc.md | 🔄 DELEGATE | Better suited for @pm | Use @pm *create-prd |
...
grep -r "\*command-name" docs/ .lmas-core/ | wc -l

# For each agent's commands:
for cmd in $(grep -E "^  - [a-z-]+" .lmas-core/agents/AGENT.md | cut -d':' -f1 | sed 's/  - //'); do
  count=$(grep -r "\*$cmd" docs/ .lmas-core/ 2>/dev/null | wc -l)
  echo "$cmd: $count"
done
```
...
```yaml
- create {type} {name}: Create LMAS component
