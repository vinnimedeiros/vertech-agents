...
$allTasks = Get-ChildItem .lmas-core/tasks/*.md | Where-Object { $_.Name -notlike '*v1-backup*' } | ForEach-Object { $_.Name }
...
foreach ($task in $allTasks) {
  $result = node .lmas-core/scripts/validate-task-v2.js ".lmas-core/tasks/$task" 2>&1
...
foreach ($task in $nonCompliantTasks) {
  ...
  node .lmas-core/scripts/migrate-task-to-v2.js ".lmas-core/tasks/$task" 2>&1 | Out-Null
