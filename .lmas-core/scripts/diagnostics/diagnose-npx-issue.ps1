# Diagnóstico de problema com npx lmas-core
# Execute este script no PC com problema

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "   Diagnóstico NPX lmas-core" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[4] Baixando lmas-core manualmente..." -ForegroundColor Yellow
$tempDir = "$env:TEMP\lmas-test-$(Get-Random)"
New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
Set-Location $tempDir

Write-Host "    Diretório: $tempDir"
npm pack lmas-core@2.2.0 2>&1 | Out-Null

if (Test-Path "lmas-core-2.2.0.tgz") {
    Write-Host "    Download: OK" -ForegroundColor Green

    # Extrair
    tar -xzf lmas-core-2.2.0.tgz 2>&1 | Out-Null

    if (Test-Path "package/bin/lmas.js") {
        Write-Host "    Extração: OK" -ForegroundColor Green

        # Tentar executar
        Write-Host ""
        Write-Host "[5] Executando bin/lmas.js --version:" -ForegroundColor Yellow
        try {
            $result = node package/bin/lmas.js --version 2>&1
        ...
        Write-Host "    Executando: node package/bin/lmas.js --help"
        node package/bin/lmas.js --help 2>&1

    } else {
        Write-Host "    Extração: FALHOU - bin/lmas.js não encontrado" -ForegroundColor Red
...
Write-Host "4. Tente novamente: npx lmas-core@latest"
Write-Host ""
