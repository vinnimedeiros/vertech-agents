#!/bin/bash
#
# LMAS Framework Documentation Migration Script
#
# Purpose: Migrate framework docs from lmas-fullstack to lmas-core (REPO 1)
...
#   --target-repo    Path to lmas-core repository (default: ../lmas-core)
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  LMAS Framework Documentation Migration Script             ║${NC}"
...
if [ ! -d "$SOURCE_DOCS" ]; then
  ...
  echo "  Please run this script from the lmas-fullstack root directory"
  exit 1
fi
...
if [ ! -d "$TARGET_REPO" ]; then
  ...
  echo "  Please clone lmas-core first or specify correct path with --target-repo"
  exit 1
fi
...
**Source:** lmas-fullstack/docs/framework/
**Target:** lmas-core/docs/framework/
**Story:** 6.1.2.6 - Framework Configuration System

## Migration Status

✅ Framework documentation successfully migrated from lmas-fullstack to lmas-core (REPO 1)

## Files Migrated

- coding-standards.md - Official LMAS coding standards
...
- lmas-fullstack maintains copies in `docs/framework/` (will be removed Q3 2026)
- lmas-fullstack maintains legacy copies in `docs/architecture/` (will be removed Q3 2026)
...
if [ "$DRY_RUN" = false ] && [ -d "$TARGET_REPO/.git" ]; then
  ...
  echo "    git commit -m \"docs: migrate framework documentation from lmas-fullstack\""
  echo ""
  echo "    Story: 6.1.2.6 - Framework Configuration System"
  echo "    Migrated: coding-standards.md, tech-stack.md, source-tree.md"
  echo "    Source: lmas-fullstack/docs/framework/"
  echo ""
...
echo "    4. Commit changes to lmas-core repository"
echo "    5. Update lmas-fullstack deprecation warnings (Q3 2026)"
echo ""
