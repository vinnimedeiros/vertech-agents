#!/bin/bash
# ============================================
# LMAS PM Orchestration Script (pm.sh)
...
#   --output    - Custom output file path (default: /tmp/lmas-output-{timestamp}.md)
#
# Environment Variables:
#   LMAS_OUTPUT_DIR   - Directory for output files (default: /tmp)
#   LMAS_DEBUG        - Enable debug logging (default: false)
#   LMAS_TIMEOUT      - Timeout in seconds (default: 300)
...
# Configuration
OUTPUT_DIR="${LMAS_OUTPUT_DIR:-/tmp}"
DEBUG="${LMAS_DEBUG:-false}"
TIMEOUT="${LMAS_TIMEOUT:-300}"
CLAUDE_CMD="${CLAUDE_CMD:-claude}"
INLINE_MODE="${LMAS_INLINE_MODE:-false}"
...
show_help() {
  cat << EOF
LMAS Multi-Modal Orchestration Script v${VERSION}
...
Environment Variables:
  LMAS_OUTPUT_DIR    Output directory (default: /tmp)
  LMAS_DEBUG         Enable debug mode (default: false)
  LMAS_TIMEOUT       Timeout in seconds (default: 300)
  LMAS_INLINE_MODE   Run without a visual terminal (default: false)
  CLAUDE_CMD         Claude CLI command (default: claude)
...
setup_paths() {
  ...
  if [[ -n "$CUSTOM_OUTPUT" ]]; then
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/lmas-output-${timestamp}.md"
  fi

  LOCK_FILE="${OUTPUT_DIR}/lmas-lock-${timestamp}.lock"
...
spawn_inline() {
  ...
  # Build the command
  local output=""
  output+="=== LMAS Agent Session (Inline) ===$(printf '\n')"
...
spawn_terminal() {
  ...
  local full_cmd="echo '=== LMAS Agent Session ===' && "
...
main() {
  ...
  log_info "LMAS Multi-Modal Orchestration Script v${VERSION}"
