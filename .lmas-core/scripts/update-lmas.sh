#!/bin/bash
# LMAS Framework Update - v5.2 (Optimized)
#
# LOGIC:
...
#
# Usage: bash .lmas-core/scripts/update-lmas.sh

set -e

echo "⚡ LMAS Update v5.2"
echo ""
...
# Validate: clean working tree for .lmas-core
if [ -n "$(git status --porcelain .lmas-core/)" ]; then
  echo "❌ Commit .lmas-core changes first:"
  git status --short .lmas-core/
  echo ""
  echo "Run: git add .lmas-core && git commit -m 'your message'"
  exit 1
fi
...
git sparse-checkout set .lmas-core
cd - > /dev/null

echo "✅ Fetched upstream"
echo ""

# Build file lists (relative paths without .lmas-core/ prefix)
echo "📋 Scanning files..."
find .lmas-core -type f | sed 's|^\.lmas-core/||' | sort > "$TEMP_DIR/local-files.txt"
find "$TEMP_DIR/upstream/.lmas-core" -type f | sed "s|^$TEMP_DIR/upstream/\.lmas-core/||" | sort > "$TEMP_DIR/upstream-files.txt"

# Get git-tracked files (for DELETE detection) - ONE call instead of N calls
git ls-files .lmas-core | sed 's|^\.lmas-core/||' | sort > "$TEMP_DIR/tracked-files.txt"
...
while IFS= read -r rel_path; do
  if ! cmp -s ".lmas-core/$rel_path" "$TEMP_DIR/upstream/.lmas-core/$rel_path"; then
...
while IFS= read -r rel_path; do
  ...
  cp -a ".lmas-core/$rel_path" "$TEMP_DIR/local-only/$rel_path"
...
if [ -s "$REPORT_DELETED" ]; then
  while IFS= read -r rel_path; do
    rm -f ".lmas-core/$rel_path"
  done < "$REPORT_DELETED"
fi

# Copy all upstream files (creates new + overwrites existing)
rsync -a "$TEMP_DIR/upstream/.lmas-core/" ".lmas-core/"

# Restore local-only files
if [ -d "$TEMP_DIR/local-only" ] && [ "$(ls -A "$TEMP_DIR/local-only" 2>/dev/null)" ]; then
  rsync -a "$TEMP_DIR/local-only/" ".lmas-core/"
fi

# Clean empty directories
find .lmas-core -type d -empty -delete 2>/dev/null || true
...
echo ""
echo "Choose:"
echo "  ✅ Apply:  git add .lmas-core && git commit -m 'chore: sync LMAS framework'"
echo "  ❌ Cancel: git checkout -- .lmas-core/"
echo ""
